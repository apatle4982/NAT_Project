<?php
/**
  * @var \App\View\AppView $this
  * @var \App\Model\Entity\FilesVendorAssignment[]|\Cake\Collection\CollectionInterface $FilesVendorAssignment
  */
?>
<style>
.grey-out-color{
    background:#eee !important;
    border-color:#c8bfbf !important;
    color:#000 !important;
}
</style>
<!-- ================================================================ -->

<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
    <?php // $this->Form->create($FilesVendorAssignment, ['horizontal'=>true]) ?>
	<?= $this->Form->create($FilesVendorAssignment, ['horizontal' => true, 'id' => 'emailForm']) ?>
        <div class="col-lg-12">

        <div class="card">
            <!--div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Filter By</h4>
            </div-->
            <div class="card-body">
                <div class="live-preview ">
                    <div class="row long-lbl-frm">
                    <div class="col-xxl-8 col-md-7 col-sm-12">

                    <h2>Partner</h2>
                    <div class="row">
                        <div class="col-xxl-4 col-md-4 col-sm-12">
                            <div class="row">
                                <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><?= (isset($partnerMapField['mappedtitle']['company_id'])? ($partnerMapField['mappedtitle']['company_id'] != 'company_id') ? $partnerMapField['mappedtitle']['company_id']: 'Partner' : 'Partner') ?></label>

                                    <?php
                                            echo $this->Form->control('company_id', ['value' => isset($formpostdata['company_id'])? $formpostdata['company_id']: '', 'options' => $companyMsts, 'multiple' => false, 'empty' => 'Select Partner', 'class'=>'form-control','label'=>false, 'required'=>false]);
                                        ?>
                                    </div>
                                </div>
                                <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= ((isset($partnerMapField['mappedtitle']['TransactionType']) && (!empty($partnerMapField['mappedtitle']['TransactionType']))) ? $partnerMapField['mappedtitle']['TransactionType']: 'Transaction Type'); ?></strong></label>
                                    <?php
                                            echo $this->Form->control('TransactionType', [
                                                'value' => isset($formpostdata['TransactionType'])? $formpostdata['TransactionType']: '',
                                                'options' => $DocumentTypeData,
                                                'multiple' => false,
                                                'empty' => 'Select Transaction Type',
                                                'label' => [
                                                        'text' => ((isset($partnerMapField['mappedtitle']['TransactionType']) && (!empty($partnerMapField['mappedtitle']['TransactionType']))))? $partnerMapField['mappedtitle']['TransactionType']: 'Transaction Type',
                                                        'escape' => false
                                                ],
                                                'class'=>'form-control',
                                                'label'=>false,
                                                'required'=>false
                                            ]);

                                        ?>
                                    </div>
                                </div>
                                <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (isset($partnerMapField['mappedtitle']['State']) ? $partnerMapField['mappedtitle']['State']: 'State') ?></strong></label>

                                    <?php echo $this->Form->control('State', [
                                                    'label' => false,
                                                'value'=>isset($formpostdata['State'])? $formpostdata['State']: '' , 'class'=>'form-control', 'required'=>false]); ?>

                                    </div>
                                </div>



                            </div>
                        </div>
                        <div class="col-xxl-4 col-md-4 col-sm-12">
                            <div class="row">
                            <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (((isset($partnerMapField['mappedtitle']['NATFileNumber']) && (!empty(trim($partnerMapField['mappedtitle']['NATFileNumber'])))))? $partnerMapField['mappedtitle']['NATFileNumber']: 'NAT File Number')?></strong></label>
                                    <?php echo $this->Form->control('NATFileNumber', [
                                    'label' => false,
                                    'value'=>isset($formpostdata['NATFileNumber'])? $formpostdata['NATFileNumber']: '' , 'class'=>'form-control', 'required'=>false]); ?>
                                    </div>
                            </div>
                            <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (isset($partnerMapField['mappedtitle']['StreetName'])? $partnerMapField['mappedtitle']['StreetName']: 'Street Name') ?></strong></label>
                                    <?php echo $this->Form->control('StreetName', ['value'=>isset($formpostdata['StreetName'])? $formpostdata['StreetName']: '' ,
                                        'label' => false,
                                        'class'=>'form-control', 'required'=>false]); ?>
                                    </div>
                            </div>
                            <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (isset($partnerMapField['mappedtitle']['County'])? $partnerMapField['mappedtitle']['County']: 'County') ?></strong></label>
                                    <?php echo $this->Form->control('County', ['value'=>isset($formpostdata['County'])? $formpostdata['County']: '' ,
                                        'label' =>false,
                                        'class'=>'form-control', 'required'=>false]); ?>
                                    </div>
                            </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-md-4 col-sm-12">
                            <div class="row">
                                <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (isset($partnerMapField['mappedtitle']['PartnerFileNumber'])? $partnerMapField['mappedtitle']['PartnerFileNumber']: 'Partner File Number') ?></strong></label>
                                    <?php echo $this->Form->control('PartnerFileNumber', ['value'=>isset($formpostdata['PartnerFileNumber'])? $formpostdata['PartnerFileNumber']: '' ,
                                    'label' => false,
                                    'class'=>'form-control', 'required'=>false]); ?>

                                    </div>
                                </div>
                                <div class="col-xxl-12 col-md-12">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong><?= (isset($partnerMapField['mappedtitle']['Grantors'])? $partnerMapField['mappedtitle']['Grantors']: 'Grantor(s)') ?></strong></label>
                                    <?php echo $this->Form->control('Grantors', ['value'=>isset($formpostdata['Grantors'])? $formpostdata['Grantors']: '' ,
                                    'label' => false,
                                    'class'=>'form-control', 'required'=>false]); ?>
                                    </div>
                                </div>
                                <!-- <div class="col-xxl-12 col-md-12" id="attorney_block" style="display:none;">
                                    <div class="input-container-floating">
                                    <label for="basiInput" class="form-label"><strong>Attorney</strong></label>
                                    <?php //echo $this->Form->control('vendorid', ['value' => isset($formpostdata['vendorid'])? $formpostdata['vendorid']: '', 'options' => $vendorlist, 'multiple' => false, 'empty' => 'Select Attorney', 'class'=>'form-control','label'=>false]); ?>
                                    </div>
                                </div> -->
                            </div>
                        </div>
                    </div>

                    <h2>Transaction Status</h2>

                    <div class="row">
                        <div class="col-xxl-12 col-md-12">


                            <div class="input-container-floating">
                                <div class="form-check checkBox">
                                    <label class="form-check-label" for="documentreceived-pending">
                                     Pending (Not Created/Sent)
                                    </label>
                                    <?php
                                    echo $this->Form->input('DocumentReceived', [

                                        'type' => 'radio',
                                        'options' => ['pending'=>'Pending'],
                                        'required' => 'required',
                                        'label' => false,
                                        'default' => "pending",
                                        'class'=>'form-check-input'
                                        ]);
                                    ?>
                                </div>
                                <div class="form-check checkBox">
                                    <label class="form-check-label" for="documentreceived-pre_aol_status">
                                    Preliminary AOL Created
                                    </label>
                                    <?php
                                    echo $this->Form->input('DocumentReceived', [

                                        'type' => 'radio',
                                        'options' => ['pre_aol_status'=>'Assigned'],
                                        'required' => 'required',
                                        'label' => false,
                                        'default' => "pending",
                                        'class'=>'form-check-input'
                                        ]);
                                    ?>
                                </div>
                                <div class="form-check checkBox">
                                    <label class="form-check-label" for="documentreceived-final_aol_status">
                                    Final AOL Created
                                    </label>
                                    <?php
                                    echo $this->Form->input('DocumentReceived', [

                                        'type' => 'radio',
                                        'options' => ['final_aol_status'=>'Assigned'],
                                        'required' => 'required',
                                        'label' => false,
                                        'default' => "pending",
                                        'class'=>'form-check-input'
                                        ]);
                                    ?>
                                </div>

                            </div>
                            </div>
                    </div>
                </div>
                <div class="col-xxl-4 col-md-5 col-sm-12">

                    <h2>Upload/Add Specific Search</h2>

                    <div class="row">
                    <div class="col-xxl-8 col-md-7 col-sm-12">
                        <div class="row">
                            <div class="col-xxl-12 col-md-12 col-sm-12">
                                <div class="input-container-floating">
                                        <label for="basiInput" class="form-label"><strong>Date Uploaded/Added</strong></label>
                                        <div class="two-input">
                                        <div class="row">
                                            <div class="col-xxl-12 col-md-12 col-sm-12">
                                            <span class="frm-to">From:</span>
                                            <?php echo $this->Form->control('fromdate', ['label' => false, 'value'=>isset($formpostdata['fromdate'])? $formpostdata['fromdate']: '', 'placeholder' => '(yyyy-mm-dd)', 'class'=>'form-control f-control-withspan', 'required'=>false]); ?>

                                            </div>
                                            <div class="col-xxl-12 col-md-12 col-sm-12">
                                            <span class="frm-to">To:</span>
                                            <?php echo $this->Form->control('todate', ['label' => false, 'value'=>isset($formpostdata['todate'])? $formpostdata['todate']: '', 'placeholder' => '(yyyy-mm-dd) ', 'class'=>'form-control f-control-withspan', 'required'=>false]); ?>

                                            </div>
                                        </div>


                                </div>
                            </div>
                            </div>
                        </div>
                    </div>
                        <div class="col-xxl-4 col-md-4 col-sm-12">
                        <?= $this->LRS->searchCancelBtnAol('m-r') ?>

                        <?php echo $this->Form->control('sno',['type'=>'hidden','id'=>'snoId','value'=>'']); ?>
                        <?php echo $this->Form->control('docstatus',['type'=>'hidden','id'=>'docstatusId','value'=>'']); ?>

                        </div>

                                        
                    </div>
                    <div class="row" style="margin-top: 15px;">
                        <div class="col-xxl-8 col-md-8 col-sm-12">
                            <h2>About Actions:</h2> 
                            <div class="row">
                                <div style="display: flex; align-items: center; gap: 5px;">
                                        <i class="ri-mail-fill" style="color:#808080 !important;font-size:14px;"></i> <strong>Click to send Preliminary AOL Email (Partner)</strong> 
                                </div>
                                <div style="display: flex; align-items: center; gap: 5px;">
                                        <i class="ri-mail-fill" style="color:#0000ff !important;font-size:14px;"></i> <strong>Click to send Final AOL Email to Attorney</strong> 
                                </div>
                                <div style="display: flex; align-items: center; gap: 5px;">
                                        <i class="ri-mail-fill" style="color:#f1702b !important;font-size:14px;"></i><strong>Click to Submit Approved AOL to Client</strong> 
                                </div>       
                            </div> 
                        </div>                
                    </div>
                    
                    </div>
                    <div class="row">
                        <div class="col-xxl-5 col-md-5 col-sm-12">
                            <div class="aolbtns">
                            <!--<form id="aol" name="aol" action="http://" target="/files-aol-assignment/add">-->
                                <input id="assign_pre" Type="submit" value="Create Preliminary AOL" class="btn btn-primary" name="pre_aol" />
                                <input id="assign_final" Type="submit" value="Create Final AOL for Attorney" class="btn btn-primary"  name="final_aol"/>
                                <!-- <input id="assign_sub" Type="submit" value="Prepare Approved AOL for Client" class="btn btn-primary" name="submit_aol"/> -->
                            </div>  
                        </div>  
                        <!-- <div class="col-xxl-7 col-md-7 col-sm-12">
                            
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <h2>Bulk Action:</h2> 
                            <?php
                                $bulkOptions = ['pre'=>'Preliminary AOL Email','final'=>'Final AOL Email','aol_signed'=>'Submit Approved AOL to Client'];

                                echo $this->Form->control('bulk_select', ['value' => '', 'options' => $bulkOptions, 'multiple' => false, 'empty' => 'Select Bulk Action', 'class'=>'form-control','label'=>false, 'style'=>'width:150px;']);
                                echo $this->Form->control('vendorid', ['value' => '', 'options' => $vendorlist, 'multiple' => false, 'empty' => 'Select Attorney', 'class'=>'form-control','label'=>false, 'style'=>'width:150px;']);
                            ?>
                            <input id="bulk_submit" Type="button" value="Submit" class="btn btn-primary bulk_email_pop" name="bulk_submit" />
                            </div>
                        </div> -->
                    <!--</form>-->
                <!-- Button to trigger modal -->
    			    </div>
                </div>
            </div>

            </div>
            </div>
        </div>
        <!-- table data -->
        <div class="card">
			<div class="card-body">
				<div class="live-preview">
				<!-- Records Listing -->
				<?php echo $this->element('checkin_records_list'); ?>
				</div>
			</div>
        </div>
    </div>
   <?= $this->Form->end() ?>
   <!-- Email Modal -->
<?= $this->Form->create(null, ['url' => ['controller' => 'Pdf', 'action' => 'sendAolEmail']]) ?>
<div class="modal fade email_popup" id="emailModal" tabindex="-1" aria-labelledby="emailModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="emailModalLabel">Send Email</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <!--<?= $this->Form->create(null, ['url' => ['controller' => 'Emails', 'action' => 'send'], 'type' => 'file']) ?>-->
                    <div class="mb-3">
                        <?= $this->Form->control('email', [
                            'label' => 'Email Address',
                            'class' => 'form-control',
                            'type' => 'email',
                            'id' => 'popup_email',
                            'required' => true
                        ]); ?>
                        <span class="asgn_error"></span>

                        <?= $this->Form->control('bulk_email', [
                            'label' => false,
                            'class' => 'form-control',
                            'type' => 'textarea',
                            'id' => 'bulk_email',
                            'style'=>'display:none;height:60px;',
                            'disabled' =>true
                        ]); ?>
                    </div>
                    <div class="mb-3">
                        <?= $this->Form->control('subject', [
                            'label' => 'Subject',
                            'class' => 'form-control',
                            'required' => true
                        ]); ?>
                    </div>
                    <div class="mb-3">
                        <?= $this->Form->control('body', [
                            'label' => 'Message',
                            'class' => 'form-control',
                            'type' => 'textarea',
                            'style' => 'height:60px;',
                            'required' => true,
                        ]); ?>
                    </div>
                    <div class="mb-3" id="attachment" style='display: none'>
					<a href='' target='_blank'>View PDF</a>
                    </div>
                    <div class="modal-footer">
						<input id="email_btn_id" type="hidden" name="email_btn_id" value="">
						<input id="email_type" type="hidden" name="email_type" value="">
                        <input id="email_send_type" type="hidden" name="email_send_type" value="">
                        <input id="bulk_record_id" type="hidden" name="bulk_record_id[]" value="">
                        <button type="submit" class="btn btn-success" id="send-email">Send Email</button>
                        <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                    </div>
                <!--<?= $this->Form->end() ?>-->
            </div>
        </div>
        <div class="text-center modal-loader" style="display:none;">
            <!-- White spinner -->
            <div class="spinner-border text-light" role="status" style="width: 3rem; height: 3rem;">
                <span class="visually-hidden">Loading...</span>
            </div>
            <!-- White text -->
            <div class="mt-2 text-white">Please wait...</div>
        </div>
    </div>
</div>
</div>
<?= $this->Form->end() ?>

<!-- Bulk Email Modal -->
<?= $this->Form->create(null, ['url' => ['controller' => 'Pdf', 'action' => 'bulkSendAolEmail']]) ?>
<div class="modal fade email_popup" id="bulkEmailModal" tabindex="-1" aria-labelledby="bulkEmailModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="bulkEmailModalLabel">Bulk Email</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
               
                    <div class="mb-3">
                        <?= $this->Form->control('bulk_subject', [
                            'label' => 'Subject',
                            'class' => 'form-control',
                            'required' => true
                        ]); ?>
                    </div>
                    <div class="mb-3">
                        <?= $this->Form->control('bulk_body', [
                            'label' => 'Message',
                            'class' => 'form-control',
                            'type' => 'textarea',
                            'style' => 'height:60px;',
                            'required' => true
                        ]); ?>
                    </div>
                    <!-- <div class="mb-3" id="attachment" style='display: none'>
					<a href='' target='_blank'>View PDF</a>
                    </div> -->
                    <div class="modal-footer">
						<input id="bulk_record_id_old" type="hidden" name="bulk_record_id_old[]" value="">
						<input id="bulk_email_type" type="hidden" name="bulk_email_type" value="">
                        <input id="bulk_vendorid" type="hidden" name="bulk_vendorid" value="">
                        <button type="submit" class="btn btn-success">Send Email</button>
                        <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                    </div>
                
            </div>
        </div>
    </div>
</div>
</div>
<?= $this->Form->end() ?>

<!-- Barcode Modal -->

<!-- Barcode Modal helper -->
<?php //echo $this->LRS->showBarCodeModelPop();
//echo "esssss<pre>";print_r($vendor_services);echo "</pre>";exit;
echo $this->LRS->showPasswordModelPopAol();
echo $this->LRS->showBarCodeModelPop($vendorlist, $vendor_services);
		echo $this->LRS->showLockModelPop();
		 ?>


<?php $this->append('script') ?>
<script src="https://cdn.jsdelivr.net/jquery.validation/1.16.0/jquery.validate.min.js"></script>
<script type="text/javascript">
	$(document).ready(function () {

        var selectedValue = $('input[name="DocumentReceived"]:checked').val();
        disableButtons(selectedValue);//alert(selectedValue)
        // call function to load data table
        loadDataTable();

        if(selectedValue == 'final_aol_status'){
            $('#attorney_block').show();
        } 
        
        $("button.dreceived").click(function(){
            var docReceived = [];
            $.each($(".checkSingle:checkbox:checked"), function(){
                var fileno = $(this).val()
               // var doctypeval = $(this).parent().parent().find('input[type="text"]').val();
			    var doctypeval = $(this).parent().parent().find('input[name="docTypeInput"]').val();
				var documentTypeHidden = $(this).parent().parent().find('input[name="documentTypeHidden"]').val();
                var docidarray = doctypeval.split(',');
                $.each(docidarray, function( index, value ) {
                    docReceived.push(fileno+"_"+value+"_"+documentTypeHidden);
                });
            });

            if(docReceived.length == 0){
                alert("Please select at least one record");
                return false;
            }else{
                $("#docstatusId").val("dr");
                $("#snoId").val(docReceived.join(","));
                $( "#searchBtnId" ).trigger("click");
            }
        });


        $("button.dnreceived").click(function(){
            var docNotReceived = [];
            $.each($(".checkSingle:checkbox:checked"), function(){
                var fileno = $(this).val()
               // var doctypeval = $(this).parent().parent().find('input[type="text"]').val();
			    var doctypeval = $(this).parent().parent().find('input[name="docTypeInput"]').val();

                var docidarray = doctypeval.split(',');
                $.each(docidarray, function( index, value ) {
                    docNotReceived.push(fileno+"_"+value);
                });
            });

            if(docNotReceived.length == 0){
                alert("Please select at least one record");
                return false;
            }else{
                $("#docstatusId").val("dnr");
				$("#snoId").val(docNotReceived.join(","));
                $( "#searchBtnId" ).trigger("click");
            }
        });

        $(document).on('change', '.checkSingle', function() {
            var selectedValues = [];

            // Iterate over all checked checkboxes and get their values
            $('.checkSingle:checked').each(function() {
                //alert($(this).val());
                selectedValues.push($(this).val());
            });

            // Update the hidden input with comma-separated values
            $('#file_nos').val(selectedValues.join(','));
        });

        /*$(document).on("click", ".send_btn", function () {
			var newAction = "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'sendAolEmail']) ?>";
			$("#emailForm").attr("action", newAction).submit();
		});*/
        $(document).on("change", "#company-id", function (e) {

            var partnerId = $("#company-id").val();
            
            $('#emailForm').submit();
        });
        $(document).on("change", "#vendorid", function (e) {

            var vendorid = $("#vendorid").val();
            
            $('#emailForm').submit();
        });
        $(document).on("change", ".checkSingle", function (e) {

            if(selectedValue == 'final_aol_status'){

                var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                    return $(this).val();
                }).toArray();

                if(selectedValues.length > 1){
                    //$('#attorney_block').show();
                }
                else{
                    //$('#attorney_block').hide();
                }
            }
        });

		$(document).on("click", ".email_pop_btn", function (e) {

            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            var partnerId = $("#company-id").val();

            if(selectedValues.length > 1){
               
                if(partnerId === ''){
                    alert("Partner is not selected! Please select any partner for bulk 'Preliminary AOL Email'");
                }
                else{
                    alert('Do you really want to Send Bulk Pre Email?');
                    var modalElement = document.getElementById('emailModal');
                    var modal = new bootstrap.Modal(modalElement);
                    modal.show();
                    $("#attachment").hide();

                    $("#email_send_type").val("pre_email_bulk");
                    $(".asgn_error").html("");
                    $("#popup_email").val("");
                    $("#bulk_record_id").val(selectedValues);
                    $.ajax({
                        url: "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'getPartnerEmail']) ?>",
                        type: "POST",
                        data: { partner_id: partnerId },
                        dataType: "json",
                        success: function (response) {
                            if (response.success) {
                                $("#popup_email").val(response.email); // Set the email in the input field
                            } else {
                                var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'index']) ?>";
                                $(".asgn_error").html("Vendor is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
                            }
                        },
                        error: function () {
                            alert("An error occurred while fetching the email.");
                        } 
                    });
                }
                return false;
            }
            else{
                var modalElement = document.getElementById('emailModal');
                var modal = new bootstrap.Modal(modalElement);
                modal.show();
                $("#attachment").show();
                
                var fileId = $(this).closest("tr").find("input.checkSingle").val(); // Get the file ID
                $("#email_send_type").val("pre_email_single");
                $(".asgn_error").html("");
                $("#popup_email").val("");
                $.ajax({
                    url: "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'getCompanyEmail']) ?>",
                    type: "POST",
                    data: { file_id: fileId },
                    dataType: "json",
                    success: function (response) {
                        if (response.success) {
                            $("#popup_email").val(response.email); // Set the email in the input field

                            /* var emailId = $(this).closest("tr").find("input.checkSingle").val(); // Get the value of the checkbox
                            $("#email_btn_id").val(emailId);alert(emailId)
                            $("#email_type").val("pre");
                            $("#emailModalLabel").text("Preliminary AOL Email (Partner)");
                            var filePath = "<?= $this->Url->build('/files/export/aol_assignment/pdf/pre/AssignmentDetails-', ['fullBase' => true]); ?>" + emailId + ".pdf";

                            $("#attachment a").attr("href", filePath);
                            $("#attachment").show(); */
                        } else {
                            var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'index']) ?>";
                            $(".asgn_error").html("Vendor is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
                        }
                    },
                    error: function () {
                        alert("An error occurred while fetching the email.");
                    } 
                });
            }
            
		});

        $(document).on("click", ".email_pop_btn_final", function () {

            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            var partnerId = $("#company-id").val();
            var vendorId = $("#vendorid").val();

            if(selectedValues.length > 1){
               
                $.ajax({
                    url: "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'getAttorneyEmails']) ?>",
                    type: "POST",
                    data: { recordId: selectedValues },
                    dataType: "json",
                    success: function (response) {
                        /* if (response.success) {
                            $("#popup_email").val(response.email); // Set the email in the input field
                        } else {
                            var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'attindex']) ?>";
                            $(".asgn_error").html("Attorney is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
                        } */
                       if(response['attorneyEmails'].length > 0){

                            

                            if(selectedValues.length !== response['totalEmail'].length){
                                var message = "Some attorneys are missing in the selected records. Only the assigned attorneys will receive the email. Would you like to continue?";

                                if (window.confirm(message)) {
                                    let emailArray = response['attorneyEmails'];
                                    let emailText = emailArray.join(", ");
                                    document.getElementById('bulk_email').value = emailText;

                                    //alert('Do you really want to Send Bulk Final AOL Email?');
                                    var modalElement = document.getElementById('emailModal');
                                    var modal = new bootstrap.Modal(modalElement);
                                    modal.show();
                                    $("#attachment").hide();
                                    $("#popup_email").hide();
                                    $("#bulk_email").show();
                                    $('#popup_email').prop('required', false);

                                    $("#email_send_type").val("final_email_bulk");
                                    $(".asgn_error").html("");
                                    $("#popup_email").val("");
                                    $("#bulk_record_id").val(selectedValues);
                                    /* $.ajax({
                                        url: "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'fetchAttorneyEmail']) ?>",
                                        type: "POST",
                                        data: { vendor_id: vendorId },
                                        dataType: "json",
                                        success: function (response) {
                                            if (response.success) {
                                                $("#popup_email").val(response.email); // Set the email in the input field
                                            } else {
                                                var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'attindex']) ?>";
                                                $(".asgn_error").html("Attorney is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
                                            }
                                        },
                                        error: function () {
                                            alert("An error occurred while fetching the email.");
                                        } 
                                    }); */
                                }
                                else {
                                    return false;
                                } 
                            }
                            else{
                                let emailArray = response['attorneyEmails'];
                                let emailText = emailArray.join(", ");
                                document.getElementById('bulk_email').value = emailText;

                                 //alert('Do you really want to Send Bulk Final AOL Email?');
                                var modalElement = document.getElementById('emailModal');
                                var modal = new bootstrap.Modal(modalElement);
                                modal.show();
                                $("#attachment").hide();
                                $("#popup_email").hide();
                                $("#bulk_email").show();
                                $('#popup_email').prop('required', false);

                                $("#email_send_type").val("final_email_bulk");
                                $(".asgn_error").html("");
                                $("#popup_email").val("");
                                $("#bulk_record_id").val(selectedValues);
                            }  
                            
                       }
                       else{
                            alert("Sorry, no attorneys are assigned to the selected records. Bulk email cannot be sent. Please assign attorneys.");
                       }
                    },
                    error: function () {
                        alert("An error occurred while fetching the attorneys email.");
                    } 
                });
               
                return false;
            }
            else{
                var modalElement = document.getElementById('emailModal');
                var modal = new bootstrap.Modal(modalElement);
                modal.show();
                $("#attachment").show();
                $("#popup_email").show();
                $("#bulk_email").hide();
                $('#popup_email').prop('required', true);

                var fileId = $(this).closest("tr").find("input.checkSingle").val(); // Get the file ID
               
                $("#email_send_type").val("final_email_single");
                $(".asgn_error").html("");
                $("#popup_email").val("");
                $.ajax({
                    url: "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'getAttorneyEmail']) ?>",
                    type: "POST",
                    data: { file_id: fileId },
                    dataType: "json",
                    success: function (response) {
                        if (response.success) {
                            $("#popup_email").val(response.email); // Set the email in the input field
                        } else {
                            var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'attindex']) ?>";
                            $(".asgn_error").html("Attorney is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
                        }
                    },
                    error: function () {
                        alert("An error occurred while fetching the email.");
                    }
                });
            }
			/* var fileId = $(this).closest("tr").find("input.checkSingle").val(); // Get the file ID
            $(".asgn_error").html("");
            $("#popup_email").val("");
			$.ajax({
				url: "<?= $this->Url->build(['controller' => 'Pdf', 'action' => 'getAttorneyEmail']) ?>",
				type: "POST",
				data: { file_id: fileId },
				dataType: "json",
				success: function (response) {
					if (response.success) {
						$("#popup_email").val(response.email); // Set the email in the input field
					} else {
					    var link = "<?= $this->Url->build(['controller' => 'FilesVendorAssignment', 'action' => 'attindex']) ?>";
						$(".asgn_error").html("Attorney is not assigned! Please click <a href='"+link+"'>here</a> to assign.");
					}
				},
				error: function () {
					alert("An error occurred while fetching the email.");
				}
			}); */
		});

        $(document).on("click", ".email_pop_btn_submit", function () {

            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            var partnerId = $("#company-id").val();
            var vendorId = $("#vendorid").val();

            if(selectedValues.length > 1){
               
                alert('Do you really want to Send Bulk Signed AOL Email?');
                var modalElement = document.getElementById('emailModal');
                var modal = new bootstrap.Modal(modalElement);
                modal.show();

                $("#email_send_type").val("signed_email_bulk");
                $(".asgn_error").html("");
                $("#popup_email").val("");
                $("#bulk_record_id").val(selectedValues);
                return false;
            }
            else{
                var modalElement = document.getElementById('emailModal');
                var modal = new bootstrap.Modal(modalElement);
                modal.show();

                var fileId = $(this).closest("tr").find("input.checkSingle").val(); // Get the file ID
               
                $("#email_send_type").val("signed_email_single");
                $(".asgn_error").html("");
                $("#popup_email").val("");
            }
		});

		$(document).on("click", ".email_pop_btn", function () {
			var emailId = $(this).closest("tr").find("input.checkSingle").val(); // Get the value of the checkbox
			$("#email_btn_id").val(emailId);
			$("#email_type").val("pre");
            $("#emailModalLabel").text("Preliminary AOL Email (Partner)");
			var filePath = "<?= $this->Url->build('/files/export/aol_assignment/pdf/pre/AssignmentDetails-', ['fullBase' => true]); ?>" + emailId + ".pdf";

            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            if(selectedValues.length > 1){
			    $("#attachment").hide();
            } 
            else{
                $("#attachment a").attr("href", filePath);
			    $("#attachment").show();
            }
			
		}); 

        $(document).on("click", ".email_pop_btn_final", function () {
			/* $("#attachment a").attr("href", filePath);
			$("#attachment").show(); */
            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            var emailId = $(this).find("i").attr("data-bs-id");
            $("#email_btn_id").val(emailId);
			$("#email_type").val("final");

            if(selectedValues.length > 1){
			    $("#emailModalLabel").text("Bulk Final AOL Email to Attorney");
            } 
            else{
                $("#emailModalLabel").text("Final AOL Email to Attorney");
            }

			var filePath = "<?= $this->Url->build('/files/export/aol_assignment/pdf/final/AssignmentDetails-', ['fullBase' => true]); ?>" + emailId + ".pdf";

            if(selectedValues.length > 1){
			    $("#attachment").hide();
            } 
            else{
                $("#attachment a").attr("href", filePath);
			    $("#attachment").show();
            }
		});

        $(document).on("click", ".email_pop_btn_submit", function () {
            var emailId = $(this).find("i").attr("data-bs-id");
            $("#email_btn_id").val(emailId);
			$("#email_type").val("aol_signed");
			$("#emailModalLabel").text("Submit Approved AOL to Client");

			var filePath = "<?= $this->Url->build('/files/export/aol_assignment/pdf/aol_signed/AssignmentDetails-', ['fullBase' => true]); ?>" + emailId + ".pdf";

			/* $("#attachment a").attr("href", filePath);
			$("#attachment").show(); */
            var selectedValues = $('input[name="checkAll[]"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            if(selectedValues.length > 1){
			    $("#attachment").hide();
            } 
            else{
                $("#attachment a").attr("href", filePath);
			    $("#attachment").show();
            }
		});

        $(document).on('click', 'input[name="DocumentReceived"]', function() {
            $(this).closest('form').submit();
        });

        /**
         * Start loader When Click On Send Email Button
         */
        $(document).on("click", "#send-email", function (e) {

            if($('#popup_email').val() !== '' && $('#subject').val() !== '' && $('#body').val() !== '')
            {
                $('.modal-loader').show();
                $('.modal-content').hide();

                $('.modal-dialog').addClass('modal-dialog-centered d-flex justify-content-center');
            }
        }) 
        /**
         * End loader Code When Click On Send Email Button
         */

        /**
         * Start Bulk Action Validation
         */
        var validator = $("#emailForm").validate({
            rules: {
                bulk_select: {
                    required: true
                },
                vendorid: {
                    required: true
                }
            },
            messages: {
                bulk_select: {
                    required: "Please select a bulk action"
                },
                vendorid: {
                    required: "Please select an attorney"
                }
            },
            onsubmit: false,
            errorPlacement: function (error, element) {
                error.insertAfter(element);
            },
            showErrors: function (errorMap, errorList) {
                this.defaultShowErrors(); // show normally

                // Auto-hide only the labels
                setTimeout(function () {
                    $("label.error").fadeOut();
                }, 3000);
            }
        });

        $(document).on("click", ".bulk_email_pop", function () {
            
            var selectedValues = $('input[type="checkbox"]:checked').map(function() {
                return $(this).val();
            }).toArray();

            var bulkEmailType = $("#bulk-select").val();
            var buklEailVendorId = $("#vendorid").val();
            
            $("#bulk_record_id").val(selectedValues);
            $("#bulk_email_type").val(bulkEmailType);
            $("#bulk_vendorid").val(buklEailVendorId);

            if (validator.element("#bulk-select") && validator.element("#vendorid")) {
                var modalElement = document.getElementById('bulkEmailModal');
                var modal = new bootstrap.Modal(modalElement);
                modal.show();

                if(bulkEmailType == 'pre'){
                    $("#bulkEmailModalLabel").text("Preliminary AOL Bulk Email (Partner)");
                }
                if(bulkEmailType == 'final'){
                    $("#bulkEmailModalLabel").text("Final AOL Bulk Email to Attorney");
                }
            } 
           
		});
        /**
         * End Bulk Action Validation
         */
    });

    function loadDataTable(){



        var formdata = {'formdata':<?php echo json_encode($formpostdata);?>,'is_index':'Y'};
        var columndata =<?php echo $dataJson;?>;
		var columndata =[{"data": "Checkbox","orderable": false,"bSortable": false,"mData": "Checkbox"},{"data": "FileNo","orderable": true,"bSortable": true,"mData": "FileNo"},{"data": "PartnerFileNumber","orderable": true,"bSortable": true,"mData": "PartnerFileNumber"},{"data": "TransactionType","orderable": true,"bSortable": true,"mData": "TransactionType"},{"data": "Grantors","orderable": true,"bSortable": true,"mData": "Grantors"},{"data": "StreetName","orderable": true,"bSortable": true,"mData": "StreetName"},{"data": "County","orderable": true,"bSortable": true,"mData": "County"},{"data": "State","orderable": true,"bSortable": true,"mData": "State"},{"data": "DocStatus","orderable": false,"bSortable": false,"mData": "DocStatus"},{"data": "DocStatus1","orderable": false,"bSortable": false,"mData": "DocStatus1"},{"data": "DocStatus2","orderable": false,"bSortable": false,"mData": "DocStatus2"},{"data": "DateAdded","orderable": true,"bSortable": true,"mData": "DateAdded"},{"data": "Actions","orderable": false,"bSortable": false,"mData": "Actions"}];
        //console.log(columndata);
        $('#datatable_example').DataTable({
            "lengthMenu": [[10, 25, 50, 100, -1],[10, 25, 50, 100, 'All']],
			"processing": true,
            "pageLength": 10,
            "serverSide": true,
            "searching": false,
            "dom": 'Blfrtip',
			      "buttons": [{ extend: 'csv', text: 'Export CSV', exportOptions: { columns: ':visible:not(:first-child):not(:last-child)' } }],
            "ajax": {
                url : '<?= $this->Url->build(["controller" => $this->request->getParam('controller'),"action" => "ajaxList".ucfirst($this->request->getParam('action'))]) ?>',
                data: formdata,
                type: 'POST',
				error: function (xhr, error, code) {
					if(xhr.status == 500){
						alert("Your session has expired. Would you like to be redirected to the login page?");
						window.location.reload(true); return false;
					}
				}
            },
            initComplete: function(settings, json) {
                //alert(JSON.stringify(json['attorneyEmails']));
                //let emailArray = json['attorneyEmails'];
                //let emailText = emailArray.join(", ");
            },
            "columns": columndata,
            "order": [ [1, 'asc'] ],
            "autoWidth": false,
            "columnDefs": [
                {
                    "width": "1%",
                    "targets": 0
                },
                {
                    "width": "8%",
                    "targets": 1
                },
                {
                    "width": "8%",
                    "targets": 2
                },
                {
                    "width": "8%",
                    "targets": 3
                },
                {
                    "width": "8%",
                    "targets": 4
                },
                {
                    "width": "5%",
                    "targets": 5
                },
                {
                    "width": "5%",
                    "targets": 6
                },
                {
                    "width": "5%",
                    "targets": 7
                },
                {
                    "width": "3%",
                    "targets": 8
                },
                {
                    "width": "3%",
                    "targets": 9
                },
                {
                    "width": "3%",
                    "targets": 10
                },
                {
                    "width": "10%",
                    "targets": 11
                },
                {
                    "width": "10%",
                    "targets": 12
                }
            ],
            createdRow: function( row, data, dataIndex ) {
				/*if ( data['ECapable'] == "Both SF & CSC" ) {
					$(row).addClass( 'bothColor' );
				} else if( data['ECapable'] == "SF" ) {
					$(row).addClass( 'sfColor' );
				} else if(data['ECapable'] == 'CSC') {
					$(row).addClass( 'cscColor' );
				} else if ( data['lock_status'] == 1 ) {
					$(row).addClass( 'disabledColor' );
				} */
			}
        });
    }

	function getBarcode(obj,val1,val2){
		if(obj.checked == true){

			$.ajax({
				type: "POST",
				url: '<?= $this->Url->build(["controller" => $this->request->getParam('controller'),"action" =>  "generateBarcode"]) ?>',
				data: {"fileno": val1, "doctype": val2},
				success: function(data){
					$('#printThis').html(data);
					jQuery('#myModal').modal('show');
				},
				error: function (xhr, error, code) {
					if(xhr.status == 500){
						alert("Your session has expired. Would you like to be redirected to the login page?");
						window.location.reload(true); return false;
					}
				}
			});
		}
	}

	function PrintElem(elem)
	{
		var mywindow = window.open('', 'PRINT', 'height=600,width=800');
		mywindow.document.write('<html><head><title>' + document.title  + '</title>');
		mywindow.document.write('</head><body >');
		mywindow.document.write('<h1>' + document.title  + '</h1>');
		mywindow.document.write(document.getElementById(elem).innerHTML);
		mywindow.document.write('</body></html>');

		mywindow.document.close(); // necessary for IE >= 10
		mywindow.focus(); // necessary for IE >= 10*/

		mywindow.print();
		mywindow.close();

		//return true;
	}


	function openLockModel(recId,  lock_status){

        $('#lockChechinId').val(recId);
        $('#lock_status').val(lock_status);
        jQuery('#myModalLock').modal('show');
        return false;
	}

    function saveLockRecord(element){

        var lockChechinId = $('#'+element).find('#lockChechinId').val();
        var lock_status = $('#'+element).find('#lock_status').val();
        var lock_comment = $('#'+element).find('#lock_comment').val();

        $.ajax({
            type: "POST",
            url: '<?= $this->Url->build(["controller"=> "FilesVendorAssignment", "action" => "ajaxLockRecord"]) ?>',
            data: {"chechinId":lockChechinId, "lock_status":lock_status, "lock_comment":lock_comment},
            success: function(data){
                $('#'+element).find("input[type=text], input[type=hidden],textarea").val("");

               //jQuery('#myModalLock').find('.msg-suceess').html('Record lock status updated!');
               jQuery('#myModalLock').modal('hide');

               $("#datatable_example").DataTable().destroy();
                // call function to load data table
                loadDataTable();
            },
				error: function (xhr, error, code) {
					if(xhr.status == 500){
						alert("Your session has expired. Would you like to be redirected to the login page?");
						window.location.reload(true); return false;
					}
				}
        });
        return false;
	}
	function openPasswordModel(Id){
		$('#checkinId').val(Id);
        jQuery('#myModalPassword').modal('show');
        return false;
	}
    function disableButtons(radioType){
       //pending pre_aol_status final_aol_status
        if(radioType === 'pending'){
            $('#assign_final').prop('disabled', true);
            $('#assign_final').addClass('grey-out-color');

            $('#assign_sub').prop('disabled', true);
            $('#assign_sub').addClass('grey-out-color');
        }
        if(radioType === 'pre_aol_status'){

            $('#assign_pre').val('Update Preliminary AOL');

            $('#assign_sub').prop('disabled', true);
            $('#assign_sub').addClass('grey-out-color');
        }
        if(radioType === 'final_aol_status'){
            $('#assign_pre').prop('disabled', true);
            $('#assign_pre').addClass('grey-out-color');

            $('#assign_final').val('Update Final AOL for Attorney');

            //$('#assign_sub').prop('disabled', true);
            //$('#assign_sub').addClass('grey-out-color');
        }
    }
</script>


<?php $this->end() ?>
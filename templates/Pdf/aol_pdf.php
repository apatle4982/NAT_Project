...
<div class="container">
    ...
    <table>
        <tr>
            <td class="bold"><b>Property Address:</b> <?= h($exam["OfficialPropertyAddress"]) ?><?= (h($exam["OfficialPropertyCity"])) ? ', '.h($exam["OfficialPropertyCity"]): '' ?><?= (h($exam["OfficialPropertyState"])) ? ', '.h($exam["OfficialPropertyState"]): '' ?><?= (h($exam["OfficialPropertyCounty"])) ? ', '.h($exam["OfficialPropertyCounty"]): '' ?></td>
            <!-- <td class="bold"><b>Borrower(s):</b> <?= h($fmd_data["MortgagorGrantors"]) ?><br><?= h($fmd_data["StreetNumber"]." ".$fmd_data["StreetName"]." ".$fmd_data["City"]." ".$fmd_data["County"]." ".$fmd_data["State"]." ".$fmd_data["Zip"]) ?></td> -->
             ...
             <td class="bold"><b>Borrower(s):</b> <?= h($vesting_attributes[0]['VestedAsGrantee']) ?></td>
        </tr>
        <tr>
            ...
            <td class="bold"><b>Lender:</b> <?= h($open_mortgage_attributes[0]['OpenMortgageLenderMortgagee']) ?></td>

            <td class="bold"><b>Loan Number:</b> <?= h($fmd_data["PartnerFileNumber"]) ?></td>
        </tr>
        <tr>
    ...
    <div class="row">
		<?php $vestinfos = json_decode($exam["vesting_attributes"]);
			if(count($vestinfos) > 0){
				foreach($vestinfos as $vestinfo){ //echo "<pre>";print_r($vestinfo);echo "</pre>";exit;
        ?>
				<?php if ($vestinfo->VestingDeedType) { ?>
					<div class="col">Deed Type: <?= h($vestinfo->VestingDeedType) ?></div>
				<?php } ?>
				<?php if ($vestinfo->VestingConsiderationAmount) { ?>
					<div class="col">Consideration Amount: <?= h($vestinfo->VestingConsiderationAmount) ?></div>
				<?php } ?>
				
				<?php if ($vestinfo->VestedAsGrantee) { ?>
					<div class="col">Vested As (Grantee): <?= h($vestinfo->VestedAsGrantee) ?></div>
				<?php } ?>
				
				<?php if ($vestinfo->VestingGrantor) { ?>
					<div class="col">Grantor: <?= h($vestinfo->VestingGrantor) ?></div>
				<?php } ?>
				
				<?php if ($vestinfo->VestingDated) { ?>
					<div class="col">Dated: <?= h($vestinfo->VestingDated) ?></div>
				<?php } ?>

				<?php if ($vestinfo->VestingRecordedDate) { ?>
					<div class="col">Recorded Date: <?= h($vestinfo->VestingRecordedDate) ?></div>
				<?php } ?>
				<?php if ($vestinfo->VestingBookPage) { ?>
					<div class="col">Book/Page: <?= h($vestinfo->VestingBookPage) ?></div>
				<?php } ?>

				<?php if ($vestinfo->VestingInstrument) { ?>
					<div class="col">Instrument #: <?= h($vestinfo->VestingInstrument) ?></div>
				<?php } ?>
				
				<?php if ($vestinfo->VestingComments) { ?>
					<div class="col">Comments: <?= h($vestinfo->VestingComments) ?></div>
				<?php } ?>
...
<div class="section1">
    ...
    <div class="row">
		<?php $open_mortgage_info = json_decode($exam["open_mortgage_attributes"]);
			if(count($open_mortgage_info) > 0){
				foreach($open_mortgage_info as $open_mortgage){ //echo "<pre>";print_r($vestinfo);echo "</pre>";exit;
        ?>
				<?php if ($open_mortgage->OpenMortgageAmount) { ?>
					<div class="col">Amount $: <?= h($open_mortgage->OpenMortgageAmount) ?></div>
				<?php } ?>
				<?php if ($open_mortgage->VestingConsiderationAmount) { ?>
					<div class="col">Consideration Amount: <?= h($open_mortgage->VestingConsiderationAmount) ?></div>
				<?php } ?>
				
				<?php if ($open_mortgage->OpenMortgageDated) { ?>
					<div class="col">Dated: <?= h($open_mortgage->OpenMortgageDated) ?></div>
				<?php } ?>
				
				<?php if ($open_mortgage->OpenMortgageRecordedDate) { ?>
					<div class="col">Recorded (Date): <?= h($open_mortgage->OpenMortgageRecordedDate) ?></div>
				<?php } ?>
				
				<?php if ($open_mortgage->OpenMortgageBookPage) { ?>
					<div class="col">Book/Page: <?= h($open_mortgage->OpenMortgageBookPage) ?></div>
				<?php } ?>

				<?php if ($open_mortgage->OpenMortgageInstrument) { ?>
					<div class="col">Instrument #: <?= h($open_mortgage->OpenMortgageInstrument) ?></div>
				<?php } ?>
				<?php if ($open_mortgage->OpenMortgageBorrowerMortgagor) { ?>
					<div class="col">Borrower (Mortgagor): <?= h($open_mortgage->OpenMortgageBorrowerMortgagor) ?></div>
				<?php } ?>

				<?php if ($open_mortgage->OpenMortgageLenderMortgagee) { ?>
					<div class="col">Lender (Mortgagee): <?= h($open_mortgage->OpenMortgageLenderMortgagee) ?></div>
				<?php } ?>
				
				<?php if ($open_mortgage->OpenMortgageTrustee1) { ?>
					<div class="col">Trustee 1: <?= h($open_mortgage->OpenMortgageTrustee1) ?></div>
				<?php } ?>
				
				<?php if ($open_mortgage->OpenMortgageTrustee2) { ?>
					<div class="col">Trustee 2: <?= h($open_mortgage->OpenMortgageTrustee2) ?></div>
				<?php } ?>
				<?php if ($open_mortgage->OpenMortgageComments) { ?>
					<div class="col">Comments: <?= h($open_mortgage->OpenMortgageComments) ?></div>
				<?php } ?>
...
<div class="section1">
    ...
    <div class="row">
		<?php $open_judgments_info = json_decode($exam["open_judgments_attributes"]);
			if(count($open_judgments_info) > 0){
				foreach($open_judgments_info as $open_judgments){ //echo "<pre>";print_r($vestinfo);echo "</pre>";exit;
        ?>
				<?php if ($open_judgments->OpenJudgmentsType) { ?>
					<div class="col">Type: <?= h($open_judgments->OpenJudgmentsType) ?></div>
				<?php } ?>
				<?php if ($open_judgments->OpenJudgmentsLienHolderPlaintiff) { ?>
					<div class="col">Lien Holder/Plaintiff: <?= h($open_judgments->OpenJudgmentsLienHolderPlaintiff) ?></div>
				<?php } ?>
				
				<?php if ($open_judgments->OpenJudgmentsBorrowerDefendant) { ?>
					<div class="col">Borrower/Defendant: <?= h($open_judgments->OpenJudgmentsBorrowerDefendant) ?></div>
				<?php } ?>
				
				<?php if ($open_judgments->OpenJudgmentsAmount) { ?>
					<div class="col">Amount $: <?= h($open_judgments->OpenJudgmentsAmount) ?></div>
				<?php } ?>
				
				<?php if ($open_judgments->OpenJudgmentsDateEntered) { ?>
					<div class="col">Date Entered: <?= h($open_judgments->OpenJudgmentsDateEntered) ?></div>
				<?php } ?>

				<?php if ($open_judgments->OpenJudgmentsDateRecorded) { ?>
					<div class="col">Date Recorded: <?= h($open_judgments->OpenJudgmentsDateRecorded) ?></div>
				<?php } ?>
				<?php if ($open_judgments->OpenJudgmentsBookPage) { ?>
					<div class="col">Book/Page: <?= h($open_judgments->OpenJudgmentsBookPage) ?></div>
				<?php } ?>

				<?php if ($open_judgments->OpenJudgmentsInstrument) { ?>
					<div class="col">Instrument #: <?= h($open_judgments->OpenJudgmentsInstrument) ?></div>
				<?php } ?>
				
				<?php if ($open_judgments->OpenJudgmentsComments) { ?>
					<div class="col">Comments: <?= h($open_judgments->OpenJudgmentsComments) ?></div>
				<?php } ?>
...
<div class="section1">
    <h3>Legal</h3>
	<div class="row">
        <?php if ($exam["LegalDescription"]) { ?>
            <div>Legal Description: <?= h($exam["LegalDescription"]) ?></div>
        <?php } ?>
	</div>
...
<div class="footer">
    ...
    <div style="text-align: right;">
        <img src="<?=$signature_img;?>" alt="signature" style="width: 200px; height: 40px;">
    </div>
    <?php } ?>
</div>
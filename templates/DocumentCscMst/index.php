 ...
 * @var iterable<\App\Model\Entity\DocumentCscMst> $documentCscMst
 */
?>
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h5 class="card-title mb-0 flex-grow-1"></h5>
                 <div> 
                    <?= $this->Html->link(__('Add Document SCS'), ['action' => 'add'], ['class' => 'btn btn-primary']) ?>
                </div>
            </div> 
            <div class="card-body">
				<table id="dataTables-example" class="display table table-bordered" style="width:100%">
					...
					<tr>
						...
						<th><?= __('CSC Document Type Name') ?></th>
...
$(document).ready(function () {
	...
	$('#dataTables-example').DataTable({
		...
		"ajax": {
			url : '<?= $this->Url->build(["controller" => "DocumentCscMst","action" => "ajaxListIndex"]) ?>',
			data: formdata,
			type: 'POST',

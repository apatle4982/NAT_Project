        ...
        <div class="card">
            <div class="card-header d-flex align-items-center">
                <h5 class="card-title mb-0 flex-grow-1"></h5>
                 <div> 
                    <?= $this->Html->link(__('Add Document Type'), ['action' => 'add'], ['class' => 'btn btn-primary']) ?>
                </div>
            </div> 
            <div class="card-body">
				<table id="dataTables-example" class="display table table-bordered" style="width:100%">
					<thead>
						<tr>
							...
							<th><?= __('Title') ?></th>
							<th><?= __('CountyCal Document Type') ?></th>
							<th><?= __('CSC Document Type') ?></th>
							<th><?= __('Simplifile Document Type') ?></th>
...
$(document).ready(function () {
	...
	$('#dataTables-example').DataTable({
		...
		"ajax": {
			url : '<?= $this->Url->build(["controller" => "documentTypeMst","action" => "ajaxListIndex"]) ?>',
			data: formdata,
			type: 'POST',

...
<!---------------------------->
 ...
 <div class="ibox-content">
 	<?php if($this->request->getParam('controller') == "FilesCheckinData") { 
		if($user_Gateway){
	?>
			<?php //$this->Form->button(__('Generate Sheet'), ['name'=>'checkinDataSheet', 'class'=>'btn btn-primary dreceived']) ?>
		<?php } else{ ?>
			<div >
				<input Type="button" value="Assign Vendor" onclick="getBarcode1()" class="btn btn-primary" />
		...
		<table class="table dataTable order-column stripe table-striped table-bordered" id="datatable_example">
			<thead>
				<tr>
					...
					<?php } ?>
					<!--<th ><?= __('ECapable') ?></th>-->
					<?php if($this->request->getParam('controller') == "FilesVendorAssignment") { ?>
	...
	<?php } // else for township   ?>
	
	<?php 
		if($this->request->getParam('controller') == "FilesCheckinData") { 
			if($user_Gateway){
	?>
			<?php //$this->Form->button(__('Generate Sheet'), ['name'=>'checkinDataSheet', 'class'=>'btn btn-primary dreceived']) ?>
		<?php } else{ ?>

...
<!---------------------------->
 ...
 <div class="ibox-content">
	...
	<?php } else{  ?>
	<!--------------------Township records ------------------------------->
    <?php if($this->request->getParam('action') == "aolindex" || $this->request->getParam('action') == "aolindexPartner") { ?>
	<div class="accountList">
		<table class="table dataTable order-column stripe table-striped table-bordered" id="datatable_example">
			<thead>
				<tr>
					<th>
					<?php echo ($is_index=='Y' || ($user_Gateway)) ? '<input type="checkbox" name="checkedAll" id="checkedAll" class="checkSingle" />' : 'Sr No.'; ?>
					</th>
		...
		<table class="table dataTable order-column stripe table-striped table-bordered" id="datatable_example">
			<thead>
				<tr>
					...
					<?php if($this->request->getParam('action') == "attindex") { ?>
						<th><?= __(isset($partnerMapField['mappedtitle']['AttorneyName'])? $partnerMapField['mappedtitle']['AttorneyName']: 'Attorney Name') ?></th>
					<?php } ?>	
...
function getBarcode1(){
          if ($('#file_nos').val().trim() === '') {
              alert('Please select records to proceed');
              return false; // prevent action if empty
          }

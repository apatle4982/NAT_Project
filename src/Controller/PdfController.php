...
public function sendAolEmail(){
    ...
    if ($this->request->is('post')) {
        ...
        //print_r($data);exit;
        if($data['email_send_type'] == 'pre_email_bulk' || $data['email_send_type'] == 'final_email_bulk' || $data['email_send_type'] == 'signed_email_bulk'){
            $this->bulkSendAolEmail();
        }
        else{
            ...
            if (!empty($filesExamReceiptNew['supporting_documentation'])){
                        ...
                        $filePath_1 = WWW_ROOT . 'supporting_documents' . DS.$filesExamReceiptNew['supporting_documentation'];
                        $filePath_2 = WWW_ROOT . 'uploads' . DS.$filesExamReceiptNew['supporting_documentation'];

                        if (file_exists($filePath_1)){
                            $supportingDocumentFilePath = $filePath_1;
                        }
                        if (file_exists($filePath_2)){
                            
                            $supportingDocumentFilePath = $filePath_2;
                        } 
                    }
            ...
            $receiptOfExam = $this->FilesExamReceipt->find()->where(['RecId' => $id])->first();
            /*$email = new Mailer('default');
            $email->setEmailFormat('html')
                //->setFrom(['info@tiuconsulting.com' => 'NAT'])
                ->setTo($data['email'])
            ...
            if (!empty($filePath) && file_exists($filePath)) {
                 ...
                 */
                if($data['email_type']=='pre'){

                     $email->addAttachments([$filePath]);
                     $email->addAttachments([$supportingDocumentFilePath]);
                }
                else{
                     $email->addAttachments([$filePath]);
                }
            ...
            if($data['email_type']=='final'){
                $emailBody = "
                        <p>Your Final AOL for File Number ".$fmd_data['PartnerFileNumber'].", ".$fmd_data['MortgagorGrantors']." is attached.</p>
                        <p>Please contact <u>customerservice@nationalattorneytitle.com</u> or call 440-222-7955 with questions or curative items.</p>
                        <p><strong>Additional Comments: </strong> ".$data['body']."</p>
                        <p>Thank you,<br>National Attorney Title, LLC </p>            
                    ";
            }
            else if($data['email_type']=='aol_signed'){
                $emailBody = "
                        <p>Your Final Insured AOL for File Number ".$fmd_data['PartnerFileNumber'].", ".$fmd_data['MortgagorGrantors']." is attached.</p>
                        <p>Please contact <u>customerservice@nationalattorneytitle.com</u> or call 440-222-7955 with questions or curative items.</p>
                        <p><strong>Additional Comments: </strong> ".$data['body']."</p>
                        <p>Thank you,<br>National Attorney Title, LLC </p>            
                    ";
                ...
                $emailBody = "
                        <p>Your Preliminary AOL for File Number ".$fmd_data['PartnerFileNumber'].", ".$fmd_data['MortgagorGrantors']." is attached.</p>
                        <p>Please contact <u>customerservice@nationalattorneytitle.com</u> or call 440-222-7955 with questions or curative items.</p>
                        <p><strong>Additional Comments: </strong> ".$data['body']."</p>
                        <p>Thank you,<br>National Attorney Title, LLC </p>            
                    ";
            ...
            if (!empty($filePath) && file_exists($filePath)) {
                // Send email
                $email->deliver($emailBody);

                /* End Client Email Template */

                $this->Flash->success('AOL Email sent successfully.');
                /* Public Notes code start */
                $currentUserId = $this->currentUser->user_id;
                $FmdData = $this->FilesMainData->find()->where(['Id' => $data['email_btn_id']])->first();
                $this->PublicNotes->insertNewPublicNotes($data['email_btn_id'], $FmdData['TransactionType'], $currentUserId, strtoupper($folder)." AOL email sent", 'Aol',true, "Aol Actions");
                /* Public Notes code end */
...
private function addMultiCellData(&$pdf, $title, $content)
{
   ...
   public function generatePdf($id = null, $flag = "", $signedAOL=false){
    $this->viewBuilder()->enableAutoLayout(false);
        $this->loadModel("FilesMainData");
	    $this->loadModel('FilesVendorAssignment');
	    $this->loadModel('Vendors');
        ...
        $pdf_folder = "pre";
        if ($flag == "final" or $flag == "finalpdf") { 
        ...
        $signature_img = "";
        if($signedAOL == true) {
            $signature_img = 'https://nat.tiuconsulting.us/files/'.'signature'.DS.'aol'.DS.'signature_'.$id.'.png';
            //$signature_img = WWW_ROOT . 'files' . DS . 'signature' . DS . 'aol' . DS .'signature_'.$id.'.png';
            //$signature_img = 'http://localhost/nat/files/'.'signature'.'/'.'aol'.'/'.'signature_'.$id.'.png';
        }

        $this->set(compact('fmd_data','fva_data','vendor_data', 'exam', 'flag', 'signature_img','open_mortgage_attributes','vesting_attributes'));
    ...
    public function generateApiPdf($id = null, $flag = "", $signedAOL=false){
        $this->viewBuilder()->enableAutoLayout(false);
        $this->loadModel("FilesMainData");
	    $this->loadModel('FilesVendorAssignment');
	    $this->loadModel('Vendors');
        ...
        $this->set(compact('fmd_data','fva_data','vendor_data', 'exam', 'flag', 'signature_img'));
    ...
    public function bulkSendAolEmail(){
        ...
        if ($this->request->is('post')) {
            $data = $this->request->getData();
            $recId = explode(',',$data['bulk_record_id'][0]);
            //print_r($recId);exit;
            ...
            if(count($checkRecordIdArr) === 0){
                $this->Flash->error('Invalid Request: Please select any record');
                return $this->redirect($this->referer());
            }
 
            if($data['email_type'] === ''){
                $this->Flash->error('Invalid Request: Please select any bulk action');
                return $this->redirect($this->referer());
            } 

            /* if($data['email'] === ''){
                $this->Flash->error('Invalid Request: Please select any Attorney');
                return $this->redirect($this->referer());
            } */ 
            ...
            foreach($recId as $id){
                ...
                if($id !== 'on' && $id !== 'email'){
                    ...
                    if($data['email_type']=='final'){
                        ...
                        $attorney = $filesAttorneyAssignment->find()->select(['vendorid'])->where(['RecId' => $id])->first();

                        if ($attorney) {
                            $vendor = $vendors->find()->select(['main_contact_email'])->where(['id' => $attorney->vendorid])->first();

                            if ($vendor) { 
                                $data['email'] = $vendor->main_contact_email;
                            }
                            else
                                {
                                continue;
                    ...
                    //$data['email_type'] = $data['bulk_email_type'];
                    $data['email_btn_id'] = $id;
                    //echo $v.'<br>';
                    ...
                    */
                    ...
                    //$attorney = $this->FilesAttorneyAssignment->find()->where(['RecId' => $id])->first(); 
                    //$attorney_dtl = [];
                    //$attorney_dtl = $this->Vendors->find()->where(['Id' => $data['bulk_vendorid']])->first();
                    /* if(!empty($attorney)){
                        $attorney_dtl = $this->Vendors->find()->where(['Id' => $attorney['vendorid']])->first(); 
                    } */
                    //$data['email'] = $attorney_dtl['main_contact_email'];
                    $fmd_data = $this->FilesMainData->find()->where(['Id' => $id])->first();
                    $receiptOfExam = $this->FilesExamReceipt->find()->where(['RecId' => $id])->first();
    ...
    public function getPartnerEmail()
    {
        ...
        $partner_id = $this->request->getData('partner_id'); // Get file ID from AJAX request

        if (!$partner_id) {
            $this->response->withType('application/json')
                ->withStringBody(json_encode(['success' => false, 'message' => 'Invalid partner ID']));
        }
        ...
        $company = $companyMstTable->find()
            ->select(['cm_email'])
            ->where(['cm_id' => $partner_id])
            ->first();

        if (!$company) {
            return $this->response->withType('application/json')
                ->withStringBody(json_encode(['success' => false, 'message' => 'Partner email not found']));
        }

        return $this->response->withType('application/json')->withStringBody(json_encode(['success' => true, 'email' => $company->cm_email]));
        //return $this->response->withType('application/json')->withStringBody(json_encode(['success' => true, 'email' => "vsurjuse@tiuconsulting.com"]));
    }
    public function fetchAttorneyEmail(){
        $this->request->allowMethod(['post']); // Allow only POST requests

        $vendor_id = $this->request->getData('vendor_id'); // Get file ID from AJAX request
        ...
        $vendor = $vendors->find()->select(['main_contact_email'])->where(['id' => $vendor_id])->first();
        //echo "<pre>";print_r($vendor);echo "</pre>";exit;
        if (!$vendor) {
            return $this->response->withType('application/json')
                ->withStringBody(json_encode(['success' => false, 'message' => 'Attorney email not found']));

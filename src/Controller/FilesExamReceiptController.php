...
class FilesExamReceiptController extends AppController
{
    ...
    private $columns_alise = [  "Checkbox" => "Checkbox",
   ...
   { 
    ...
    private function getCustomParshingData(array $data){
		...
		foreach ($data as $key => $value) {
			...
			$value['DocStatus'] = (($value['fva']['vendorid']>0)?'Yes':'Yes');
    ...
    */
    ...
    public function upload() {
        $pageTitle = 'Upload Receipt of Exam CSV';
        $this->set(compact('pageTitle'));

        // Retrieve session data for success and error IDs
        $successids = $this->request->getSession()->read('successids', []);
        $errorids = $this->request->getSession()->read('errorids', []);
        $this->request->getSession()->delete('successids');
        $this->request->getSession()->delete('errorids');
        $currentUserId = $this->currentUser->user_id;

        $companyFieldArray = array();
        if($this->request->getData('companyid')){
            $companyFieldsMap = $this->CompanyFieldsMapExam->find()->select(['cfm_fieldid', 'cfm_maptitle'])->where(['cfm_companyid' => $this->request->getData('companyid'),'cfm_maptitle !=' => ''])->enableHydration(false)->all()->toList();
            $companyImportFields = $this->CompanyImportFieldsExam->find()->where(['cif_companyid'=>$this->request->getData('companyid')])->first();
            if($companyImportFields['cif_fieldid'] == NULL){
                $link = Router::url(['controller' => 'FieldsMstExam','action' => 'importCompanyField']);
                $this->Flash->error(__('Receipt of Exam import fields are not set. <a href="{0}">Please click here to import fields</a>', $link),['escape' => false]);
                return $this->redirect(['action' => 'upload']);
            }else{
                $cif_fieldids_arr = explode(",", $companyImportFields['cif_fieldid']);
            }
            $import_fields = $this->FieldsMstExam->find()->select(['fm_id', 'fm_title'])->where(['fm_id IN' => $cif_fieldids_arr])->enableHydration(false)->all()->toList();
            if($companyFieldsMap){
                foreach($companyFieldsMap as $companyField){
                    $companyFieldArray[$companyField['cfm_fieldid']] = $companyField['cfm_maptitle'];
                }
            }
            $import_fields_org = array_column($import_fields, 'fm_title', 'fm_id');
            $mst_fields = ksort($import_fields_org);
            $companyFieldArray = array_filter($companyFieldArray, fn($value) => $value !== '' && $value !== null);
            $cifFieldIds = array_map('intval', explode(',', $companyImportFields['cif_fieldid']));
            $allowed_fields = array_intersect_key($companyFieldArray, array_flip($cifFieldIds));

            foreach ($allowed_fields as $key => $label) {
                if (isset($import_fields_org[$key])) {
                    $import_fields_org[$key] = $label;
                }
            }
            $allowed_fields_filtered = array_values($import_fields_org);
        }
    	$companies = $this->CompanyMst->companyListArray();
        $this->set(compact('successids', 'errorids', 'companies'));

        if ($this->request->is('post')) {
            $file = $this->request->getData('csv_file');
            $mediaType = $file->getClientMediaType();
            $allowedTypes = ['text/csv', 'application/xml', 'text/xml'];
            if (!in_array($file->getClientMediaType(), $allowedTypes)) {
                $this->Flash->error(__('Only CSV and XML files are allowed.'));
                return $this->redirect(['action' => 'upload']);
            }

            $uploadDir = WWW_ROOT . 'files' . DS . 'import' . DS . 'receiptofexam' . DS;
            if (!file_exists($uploadDir)) {
                mkdir($uploadDir, 0777, true); // Create directory if it does not exist
            }
            $filePath = $uploadDir . $file->getClientFilename();
            $file->moveTo($filePath); // Move the uploaded file

            if ($mediaType === 'text/csv') {
                $csvFile = fopen($filePath, 'r');

                if (!$csvFile) {
                    $this->Flash->error(__('Unable to read the CSV file.'));
                    return $this->redirect(['action' => 'upload']);
                }
                ...
                //echo "<pre>";print_r($allowed_fields_filtered);echo "</pre>";exit;
                if ($header !== $allowed_fields_filtered) {
                    fclose($csvFile);
                    unlink($filePath);
                ...
                while (($row = fgetcsv($csvFile)) !== false) {
                    ...
                    if (count($row) !== count($header)) {
                        $errorCount++;
                        $errorids[] = $row[0]; // Assuming PartnerFileNumber is the first column
                        continue;
                    }

                    $data = array_combine($header, $row);
                    //echo "allowed_fields<pre>";print_r($allowed_fields);echo "</pre>";
                    //echo "data<pre>";print_r($data);echo "</pre>";
                    $org_data=array();
                    foreach($data as $k=>$d){
                        $key = array_search($k, $import_fields_org);
                        $partner_org_field = $this->FieldsMstExam->find()->select('fm_title')->where(['fm_id' => $key])->first();
                        if($partner_org_field['fm_title'] != ""){
                            $org_data[$partner_org_field['fm_title']]= $d;
                        }else{
                            $org_data[$k]= $d;
                        }
                    }
                    $RecId = $this->FilesMainData->find()->select('Id')->where(['PartnerFileNumber' => $org_data['PartnerFileNumber']])->first();
                    //echo "data<pre>";print_r($org_data);echo "</pre>";exit;
                    if (!$data) {
                        $errorCount++;
                        $errorids[] = $data['PartnerFileNumber'];

                        $this->PublicNotes->insertNewPublicNotes($org_data['PartnerFileNumber'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload CSV - Error - ".$org_data['PartnerFileNumber'], 'Exam',true, "Receipt of exam upload");

                        continue;
                    }
                    //echo "<pre>";print_r($org_data);echo "</pre>";exit;

                    if (!$RecId) {
                        $errorCount++;
                        $errorids[] = $org_data['PartnerFileNumber'];
                    ...
                    $org_data['RecId'] = $RecId['Id'];
                    $existingRecord = $this->FilesExamReceipt->find()->where(['RecId' => $RecId['Id']])->first();

                    $org_data['vesting_attributes'] = json_encode($this->vest_attributes($org_data));
                    $org_data['open_mortgage_attributes'] = json_encode($this->mortgage_attributes($org_data));
                    $org_data['open_judgments_attributes'] = json_encode($this->judgments_attributes($org_data));
                    $org_data['import_csv'] = $filePath;
					$org_data['supporting_documentation'] = $org_data['SupportingDocument'];

                    $FmdData = $this->FilesMainData->find()->where(['PartnerFileNumber' => $org_data['PartnerFileNumber']])->first();
                    if ($existingRecord) {
                        $existingRecord = $this->FilesExamReceipt->patchEntity($existingRecord, $org_data);
                        if ($this->FilesExamReceipt->save($existingRecord)) {
                            $successCount++;
                            $successids[] = $org_data['PartnerFileNumber'];

                            $this->PublicNotes->insertNewPublicNotes($FmdData['Id'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload CSV - Success", 'Exam',true, "Receipt of exam upload");
                        } else {
                            $errorCount++;
                            $errorids[] = $org_data['PartnerFileNumber'];

                            $this->PublicNotes->insertNewPublicNotes($FmdData['Id'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload CSV - Error", 'Exam',true, "Receipt of exam upload");
                        }

						if (!empty($org_data['supporting_documentation']))
                    	{
							$filePath_1 = WWW_ROOT . 'uploads' . DS . $org_data['supporting_documentation'];
							$filePath_2 = WWW_ROOT . 'supporting_documents' . DS . $org_data['supporting_documentation'];
							
							if (!file_exists($filePath_2) && !file_exists($filePath_1)) {

								$errorids[] = $org_data['PartnerFileNumber'];
								$partnerFileNumber = $org_data['PartnerFileNumber'];
								$supportingDocumentName = $org_data['supporting_documentation'];
								$this->Flash->error(__("The supporting document '$supportingDocumentName' for NatFileNumber '$partnerFileNumber' is missing. Please upload the document from the 'Upload Supporting Document' section"));
							}	
				
						}
                    } else {
                        $newEntity = $this->FilesExamReceipt->newEntity($org_data);
                        if ($this->FilesExamReceipt->save($newEntity)) {
                            $successCount++;
                            $successids[] = $org_data['PartnerFileNumber'];
                ...
                $this->request->getSession()->write('successids', array_unique($successids));
                $this->request->getSession()->write('errorids', array_unique($errorids));

                if ($successCount > 0) {
                    $this->Flash->success(__("CSV file processed successfully. $successCount records have been added."));
                }
                if ($errorCount > 0) {
                    $this->Flash->error(__("$errorCount records failed to process. Please check the CSV format."));
                }
            }elseif (in_array($mediaType, ['application/xml', 'text/xml'])) {
                  $xml = simplexml_load_file($filePath);
                  $format = "xml";

                  if ($xml === false) {
                      unlink($filePath);
                      $this->Flash->error(__('Failed to parse XML file.'));
                      return $this->redirect(['action' => 'upload']);
                  }

                  // Use first record to get field names
                  $firstRecord = $xml->Record[0];
                  $header = [];
                  ...
                  if ($firstRecord) {
                      ...
                      $this->Flash->error(__('No records found in XML.'));
                      return $this->redirect(['action' => 'upload']);
                  }

                  // Optional: remove or comment this in production
                  //echo "<pre>";print_r($header);echo "</pre>";
                  //echo "import_fields_org<pre>";print_r($import_fields_org);echo "</pre>"; exit;

                  if ($header !== $allowed_fields_filtered) {
                      $this->Flash->error(__('XML does not match the required format.'));
                      return $this->redirect(['action' => 'upload']);
                  }
                  ...
                  foreach ($xml->Record as $record) {
                      ...
                      foreach ($data as $k => $d) {
                          ...
                          if (!empty($partner_org_field) && $partner_org_field['fm_title'] != "") {
                              $org_data[$partner_org_field['fm_title']] = $d;
                          } else {
                              $org_data[$k] = $d;
                          }
                      ...
                      // Defensive check: if org_data or PartnerFileNumber missing
                      if (empty($org_data) || empty($org_data['PartnerFileNumber'])) {
                          $errorCount++;
                          $errorids[] = $org_data['PartnerFileNumber'] ?? 'Unknown';
                          $FmdData = $this->FilesMainData->find()->where(['PartnerFileNumber' =>$org_data['PartnerFileNumber']])->first();
                          $this->PublicNotes->insertNewPublicNotes($org_data['PartnerFileNumber'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload XML - Error - ".$org_data['PartnerFileNumber'], 'Exam',true, "Receipt of exam upload");
                      ...
                      if ($existingRecord) {
                          $PartnerFileNumber = $org_data['PartnerFileNumber'];
                          $FmdData = $this->FilesMainData->find()->where(['PartnerFileNumber' =>$PartnerFileNumber])->first();

                          unset($org_data['PartnerFileNumber']);
                          $existingRecord = $this->FilesExamReceipt->patchEntity($existingRecord, $org_data);

                          if ($this->FilesExamReceipt->save($existingRecord)) {
                              $successCount++;
                              $successids[] = $PartnerFileNumber; // PartnerFileNumber was unset above

                              $this->PublicNotes->insertNewPublicNotes($FmdData['Id'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload XML - Success", 'Exam',true, "Receipt of exam upload");
                          } else {
                              $errorCount++;
                              $errorids[] = $PartnerFileNumber;

                              $this->PublicNotes->insertNewPublicNotes($FmdData['Id'], $FmdData['TransactionType'], $currentUserId, "Receipt of exam upload XML - Error", 'Exam',true, "Receipt of exam upload");
                          }
                      } else {
                          $newEntity = $this->FilesExamReceipt->newEntity($org_data);
                          $FmdData = $this->FilesMainData->find()->where(['PartnerFileNumber' => $org_data['PartnerFileNumber']])->first();
                  ...
                  if ($successCount > 0) {
                      $this->Flash->success(__("XML file processed successfully. $successCount records have been added."));
                  }
                  if ($errorCount > 0) {
                      $this->Flash->error(__("$errorCount records failed to process. Please check the XML format."));
                  }
              }
    ...
    public function vest_attributes($org_data){
        $org_data1[] = [
            'VestingDeedType' => $org_data['VestingDeedType'],
            'VestingConsiderationAmount' => $org_data['VestingConsiderationAmount'],
            'VestedAsGrantee' => $org_data['VestedAsGrantee'],
            'VestingGrantor' => $org_data['VestingGrantor'],
            'VestingDated' => $org_data['VestingDated'],
            'VestingRecordedDate' => $org_data['VestingRecordedDate'],
            'VestingBookPage' => $org_data['VestingBookPage'],
            'VestingInstrument' => $org_data['VestingInstrument'],
            'VestingComments' => $org_data['VestingComments']
        ];
        return $org_data1;
     }

     public function mortgage_attributes($org_data){
          $org_data2[] = [
              'OpenMortgageAmount' => $org_data['OpenMortgageAmount'],
              'OpenMortgageDated' => $org_data['OpenMortgageDated'],
              'OpenMortgageRecordedDate' => $org_data['OpenMortgageRecordedDate'],
              'OpenMortgageBookPage' => $org_data['OpenMortgageBookPage'],
              'OpenMortgageInstrument' => $org_data['OpenMortgageInstrument'],
              'OpenMortgageBorrowerMortgagor' => $org_data['OpenMortgageBorrowerMortgagor'],
              'OpenMortgageLenderMortgagee' => $org_data['OpenMortgageLenderMortgagee'],
              'OpenMortgageTrustee1' => $org_data['OpenMortgageTrustee1'],
              'OpenMortgageTrustee2' => $org_data['OpenMortgageTrustee2'],
              'OpenMortgageComments' => $org_data['OpenMortgageComments']
          ];
          return $org_data2;
     }

	public function judgments_attributes($org_data){
		$org_data3[] = [
			'OpenJudgmentsType' => $org_data['OpenJudgmentsType'],
			'OpenJudgmentsLienHolderPlaintiff' => $org_data['OpenJudgmentsLienHolderPlaintiff'],
			'OpenJudgmentsBorrowerDefendant' => $org_data['OpenJudgmentsBorrowerDefendant'],
			'OpenJudgmentsAmount' => $org_data['OpenJudgmentsAmount'],
			'OpenJudgmentsDateEntered' => $org_data['OpenJudgmentsDateEntered'],
			'OpenJudgmentsDateRecorded' => $org_data['OpenJudgmentsDateRecorded'],
			'OpenJudgmentsBookPage' => $org_data['OpenJudgmentsBookPage'],
			'OpenJudgmentsInstrument' => $org_data['OpenJudgmentsInstrument'],
			'OpenJudgmentsComments' => $org_data['OpenJudgmentsComments']
		];
		return $org_data3;
	}
	/* public function uploadSupportingDocument() {

		if ($this->request->is(['post', 'put'])) {

			$examReceiptData = $this->request->getData();
			
			if (isset($examReceiptData['supporting_documentation']) && $examReceiptData['supporting_documentation'] instanceof \Laminas\Diactoros\UploadedFile) {
				...
				if ($examReceiptData['supporting_documentation']->getClientFilename()) {
					...
					try {
						...
						// Save only the filename as a string
						$examReceiptData['supporting_documentation'] = $fileName;
						$this->Flash->success(__('Supporting document successfully uploaded'));
					} catch (\Exception $e) {
						$this->Flash->error(__('File upload failed: ' . $e->getMessage()));
	...
	public function uploadSupportingDocument()
	{
		$columns_alise = ['sr_no'=>'sr_no','doc_name'=>'doc_name','created'=>'created',"Actions" => ""];
		$columnsorder = ['sr_no','doc_name','created','secret_key'];
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if (isset($examReceiptData['supporting_documentation']) && is_array($examReceiptData['supporting_documentation'])) {
				
				$uploadedFiles = $examReceiptData['supporting_documentation'];
				$uploadedFileNames = [];

				$uploadPath = WWW_ROOT . 'supporting_documents' . DS;
				if (!is_dir($uploadPath)) {
					mkdir($uploadPath, 0777, true);
				}

				foreach ($uploadedFiles as $file) {
					if ($file instanceof \Laminas\Diactoros\UploadedFile && $file->getClientFilename()) {
						//$fileName = time() . '_' . $file->getClientFilename();
						$fileName = $file->getClientFilename();
						try {
							$file->moveTo($uploadPath . $fileName);
				...
				if (!empty($uploadedFileNames)) {
					// Save filenames (comma separated or JSON in DB if needed)
					$examReceiptData['supporting_documentation'] = json_encode($uploadedFileNames);
					$this->Flash->success(__('Supporting documents successfully uploaded'));
				} else {
					$this->Flash->error(__('No valid files uploaded.'));
				}
			} else {
				unset($examReceiptData['supporting_documentation']);
			}
	...
	/* public function listSupportingDocuments()
	{
		$this->autoRender = false;
        //echo 'vvvvvvvvvvvv  '.$this->name;exit;
		$modelname = $this->name;
		...
		$recordsTotal = 200;//$this->$modelname->find('all')->count();
		
		$uploadPath = WWW_ROOT . 'supporting_documents' . DS;
		$files = [];

		if (is_dir($uploadPath)) {
			$files = array_diff(scandir($uploadPath), ['.', '..']); // Exclude . and ..
		}
		...
		foreach($files as $value){
			...
			$childArray['sr_no'] = $count;
			$childArray['doc_name'] = $value;
			$childArray['created'] = '';
			$childArray['Actions'] = '';
			$fileArray[] = $childArray;
			$count++;
		}	
	...
	public function listSupportingDocuments()
	{
		...
		$columnsorder = ['sr_no','doc_name','created'];
		...
		// Pagination params from DataTable request
		$limit  = isset($pdata['condition']['limit']) ? (int)$pdata['condition']['limit'] : 10;
		$offset = isset($pdata['condition']['offset']) ? (int)$pdata['condition']['offset'] : 0;
		...
		foreach ($pagedFiles as $value) {
			$childArray = [];

			$filePath = $uploadPath . $value;
			$created  = file_exists($filePath) ? date("Y-m-d H:i:s", filectime($filePath)) : '';

			$deleteUrl = Router::url([
							'controller' => 'FilesExamReceipt', // adjust if different
							'action' => 'deleteSupportingDocument',
							$value
						]);
			...
			$deleteButton = '<form method="post" action="' . $deleteUrl . '" style="display:inline;">'
              ...
              . '<a href="javascript:void(0);" 
                     onclick="if(confirm(\'Are you sure you want to delete this file?\')) { this.closest(\'form\').submit(); }" 
                     style="color:red; font-size:16px;" title="Delete">'
			...
			$childArray['sr_no']    = $count;
			$childArray['doc_name'] = $value;
			$childArray['created']  = $created;
			$childArray['Actions']  = $deleteButton;
	...
	public function deleteSupportingDocument($fileName = null)
	{
		$this->request->allowMethod(['post', 'delete']); // only allow POST/DELETE

		if (!$fileName) {
			$this->Flash->error(__('Invalid file name.'));
			return $this->redirect(['action' => 'listSupportingDocuments']);
		}
		...
		if (file_exists($filePath) && is_file($filePath)) {
			if (unlink($filePath)) {
				$this->Flash->success(__('File "{0}" deleted successfully.', $fileName));
			} else {
				$this->Flash->error(__('Unable to delete the file "{0}".', $fileName));
			}
		} else {

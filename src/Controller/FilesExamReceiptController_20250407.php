    ...
    */
    ...
    public function upload() {
        ...
        if ($this->request->is('post')) {
            ...
            $expectedFields = [
                "PartnerFileNumber", "OfficialPropertyAddress", "VestingDeedType", "VestingConsiderationAmount",
            ...
            while (($row = fgetcsv($csvFile)) !== false) {
                ...
                $RecId = $this->FilesMainData->find()->select('Id')->where(['PartnerFileNumber' => $data['PartnerFileNumber']])->first();
                if (!$RecId) {
                    $errorCount++;
                    $errorids[] = $data['PartnerFileNumber'];
                    continue;
                ...
                $data1 = [
                    'VestingDeedType' => $data['VestingDeedType'],
                    'VestingConsiderationAmount' => $data['VestingConsiderationAmount'],
                    'VestedAsGrantee' => $data['VestedAsGrantee'],
                    'VestingGrantor' => $data['VestingGrantor'],
                    'VestingDated' => $data['VestingDated'],
                    'VestingRecordedDate' => $data['VestingRecordedDate'],
                    'VestingBookPage' => $data['VestingBookPage'],
                    'VestingInstrument' => $data['VestingInstrument'],
                    'VestingComments' => $data['VestingComments']
                ];

                $data2 = [
                    'OpenMortgageAmount' => $data['OpenMortgageAmount'],
                    'OpenMortgageDated' => $data['OpenMortgageDated'],
                    'OpenMortgageRecordedDate' => $data['OpenMortgageRecordedDate'],
                    'OpenMortgageBookPage' => $data['OpenMortgageBookPage'],
                    'OpenMortgageInstrument' => $data['OpenMortgageInstrument'],
                    'OpenMortgageBorrowerMortgagor' => $data['OpenMortgageBorrowerMortgagor'],
                    'OpenMortgageLenderMortgagee' => $data['OpenMortgageLenderMortgagee'],
                    'OpenMortgageTrustee1' => $data['OpenMortgageTrustee1'],
                    'OpenMortgageTrustee2' => $data['OpenMortgageTrustee2'],
                    'OpenMortgageComments' => $data['OpenMortgageComments']
                ];

                $data3 = [
                    'OpenJudgmentsType' => $data['OpenJudgmentsType'],
                    'OpenJudgmentsLienHolderPlaintiff' => $data['OpenJudgmentsLienHolderPlaintiff'],
                    'OpenJudgmentsBorrowerDefendant' => $data['OpenJudgmentsBorrowerDefendant'],
                    'OpenJudgmentsAmount' => $data['OpenJudgmentsAmount'],
                    'OpenJudgmentsDateEntered' => $data['OpenJudgmentsDateEntered'],
                    'OpenJudgmentsDateRecorded' => $data['OpenJudgmentsDateRecorded'],
                    'OpenJudgmentsBookPage' => $data['OpenJudgmentsBookPage'],
                    'OpenJudgmentsInstrument' => $data['OpenJudgmentsInstrument'],
                    'OpenJudgmentsComments' => $data['OpenJudgmentsComments']
                ];

                $data['vesting_attributes'] = json_encode($data1);
                $data['open_mortgage_attributes'] = json_encode($data2);
                $data['open_judgments_attributes'] = json_encode($data3);
                $data['import_csv'] = $filePath;

                if ($existingRecord) {
                    $existingRecord = $this->FilesExamReceipt->patchEntity($existingRecord, $data);
                    if ($this->FilesExamReceipt->save($existingRecord)) {
                        $successCount++;
                        $successids[] = $data['PartnerFileNumber'];
                    } else {
                        $errorCount++;
                        $errorids[] = $data['PartnerFileNumber'];
                    }
                } else {
                    $newEntity = $this->FilesExamReceipt->newEntity($data);
    ...
    /*public function upload(){
        ...
        if ($this->request->is('post')) {
            ...
            while (($row = fgetcsv($csvFile)) !== false) {
                ...
                $stdObject1 = json_encode($data1); $data['vesting_attributes'] = $stdObject1;
                $stdObject2 = json_encode($data2); $data['open_mortgage_attributes'] = $stdObject2;
                $stdObject3 = json_encode($data3); $data['open_judgments_attributes'] = $stdObject3;
                $data['import_csv'] = $filePath;
            ...
            $this->request->getSession()->write('successids', $successids);
            $this->request->getSession()->write('errorids', $errorids);
            if ($successCount > 0) {
                $this->Flash->success(__("CSV file processed successfully. $successCount records have been added."));
            }
            if ($errorCount > 0) {
            ...
            $this->set(compact('successids', 'errorids'));
            return $this->redirect(['action' => 'upload']);
        }

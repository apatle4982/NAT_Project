...
class FilesVendorAssignmentController extends AppController
{
	...
	*/
	...
	private  $columnsorder = [0=>'fmd.Id', 1=>'fmd.NATFileNumber', 2=>'fmd.PartnerFileNumber',3=>'fva.AttorneyName',  4=>'fva.TransactionType', 5=>'fmd.Grantors', 6=>'fmd.StreetName', 7=>'fmd.County', 8=>'fmd.State', 9=>'fva.DocumentReceived', 11=>'fva.date_updated'];
   ...
   { 
	   ...
	   /**
		* If Attorney Assignment List Page Then "Attorney Name" columns will be show other wise hide
	    */
	   if($this->request->getParam('action') !== "attindex" && $this->request->getParam('action') !== "ajaxListAttindex") {
			$keys = array_keys($this->columns_alise);
			$fourthKey = $keys[3]; // index starts from 0
			unset($this->columns_alise[$fourthKey]);

			$keys = array_keys($this->columnsorder);
			$fourthKey = $keys[3]; // index starts from 0
			unset($this->columnsorder[$fourthKey]);
    ...
    public function aolindexPartner(){ 
		$this->aolindex();
		// set page title
		$pageTitle = 'View AOL template';
		$this->set(compact('pageTitle'));
	}
    ...
    public function aolindex(){
		...
		if(isset($docstatus) && (!($this->user_Gateway))){
             ...
             foreach($pd['checkAll'] as $key=>$value){
                ...
                $FmdData = $this->FilesMainData->find()->where(['Id' => $aol['RecId']])->first();
                //echo "<pre>";print_r($FmdData['TransactionType']);echo "</pre>";exit;
                if ($this->request->is('post')) {
                  ...
                  if ($existingRecord) {
                      ...
                      $this->PublicNotes->insertNewPublicNotes($aol['RecId'], $FmdData['TransactionType'], $currentUserId, $regarding, 'Aol',true, "Aol Processing");
                  } else {
    ...
    private function setFilterQueryAol($requestFormdata, $pdata, $documentReceived='', $selectedIds=null){
		...
		if($documentReceived == "pre_aol_status"){
			$whereCondition = ['faol.pre_aol_status' => 'Y','faol.final_aol_status' => 'N'];
		}
		...
		if(isset($requestFormdata['vendorid']) && $requestFormdata['vendorid'] != ''){ 
			$whereCondition = 	array_merge($whereCondition, ['fattr.vendorid IN' => $requestFormdata['vendorid']]);
		}
    ...
    public function ajaxListAolindex(){
		...
		$data = $this->getCustomParshingDataAol($data,$documentReceived);
    ...
    public function ajaxListAolindexPartner(){
		$this->autoRender = false;
    ...
    private function getCustomParshingData(array $data){
		...
		foreach ($data as $key => $value) {
			...
			$value['DateAdded'] = $value['DateAdded']->format('Y-m-d H:i:s');
    ...
    private function getCustomParshingDataAol(array $data,$documentReceived){
		...
		foreach ($data as $key => $value) {
			...
			if($this->user_Gateway){
				...
				// onclick="getBarcode(this,'.$value["FileNo"].','.$value["TransactionType"].');"
				$value['Actions'] = $this->CustomPagination->getActionButtons($this->name,$value,['SrNo','FileNo','checkinId','TransactionType','lock_status'],$prefix = "",$hideViewButton = 10,$documentReceived);
			...
			$value['DateAdded'] = (($value['DateAdded']!='')? '<span '.$dtColor.'>'.$value['DateAdded']->format('Y-m-d H:i:s').'</span>' : '');
    ...
    private function getCustomParshingDataAtt(array $data){
		...
		foreach ($data as $key => $value) {
			...
			$value['AttorneyName'] =  (($value['AttorneyName']!='')? $value['AttorneyName'] : 'N');
		}
    ...
    public function getVendorService(){
        ...
        foreach ($vendor_services as $result) {
            ...
            <input type="radio" class="form-check-input" id="search_criteria_'.$result->id.'"  name="search_criteria" value="'.$result->id.'">
            <label class="form-check-label" for="search_criteria_'.$result->id.'">'.$result->sub_service.'</label>
        </div>';
        }
    ...
    private function sendEmail($recipientEmail, $csvDownloadLink, $csvFilePath, $cc = "", $flag = ""){
		...
		if(!empty($recipientEmail)){
			$attorney_dtl = [];
			$attorney_dtl = $this->Vendors->find()->where(['main_contact_email' => $recipientEmail])->first();
		}
		...
		if($flag=="att"){
			...
			$mailer->deliver("
				<p>Hello Sir/Madam,</p>
				<p>Please review and approve the pending Attorney Opinion Letter. Please click on the link below to be directed to the NAT website and file.</p>
				<p>Should you have any questions please contact <u>support@nationalattorneytitle.com</u> or call </p>
				<p>440-222-7955.</p>Should you have any questions please contact support@nationalattorneytitle.com or call
				<p>Thank you,</p>
				<p>National Attorney Title, LLC </p>
			");
		}
		else{
			$mailer->deliver("
				<p>Hello Sir/Madam,</p>
				<p>Attached for your review and processing, is our request for ".$attorney_dtl['name']." to complete an exam meeting the turn time, search criteria and copy requirements as agreed upon. </p>
				<p>Should you have any questions please contact <u>support@nationalattorneytitle.com</u> or call </p>
				<p>440-222-7955.</p>
				<p>Thank you,</p>
				<p>National Attorney Title, LLC </p>
...
//print_r($FilesVendorAssignment); exit;
        ...
        if ($FilesAolAssignment) {
            ...
            if ($archiveTable->save($archived)) {
                ...
                if ($this->FilesAolAssignment->delete($FilesAolAssignment)) {
                    ...
                    $this->PublicNotes->insertNewPublicNotes($id, $FmdData['TransactionType'], $currentUserId, "The AOL record has been archived and deleted.", 'Aol',true, "Aol delete action");
                } else {
	...
	public function getAttorneyEmails(){
		$this->autoRender = false;

		$pdata = $this->request->getData();
	
		$recordId = [];
		foreach ($pdata['recordId'] as $key => $value) {
			$recordId[] = $value;
		}

		/**
		 * Get Attornery Emails and Store in hidden form fields
		 */
		$query = $this->FilesAttorneyAssignment->find()
			->select(['Vendors.main_contact_email'])
			->contain(['Vendors'])
			->where(['FilesAttorneyAssignment.RecId IN' => $recordId]);

		$emails = $query->all();		
		$emailList = $emails->extract('vendor_data.main_contact_email')->toList();
		$uniqueEmails = array_values(array_unique($emailList));

		$response = array(
						"attorneyEmails" => $uniqueEmails,
						"totalEmail" => $emailList
					);

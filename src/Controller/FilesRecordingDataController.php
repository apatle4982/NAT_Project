...
class FilesRecordingDataController extends AppController
{
    ...
    private $columns_alise = [  "Checkbox" => "",
                                ...
                                "county" => "fmd.County",
                                "state" => "fmd.State",
                                "DocStatus" => "frd.DocumentReceived",
                                
                                "DateAdded" => "frd.created",
								"Actions" => ""
								
                            ];

	private  $columnsorder = [0=>'fmd.Id', 1=>'fmd.NATFileNumber', 2=>'fmd.PartnerFileNumber', 4=>'fmd.TransactionType', 5=>'fmd.Grantors', 6=>'fmd.StreetName', 7=>'fmd.county', 8=>'fmd.State', 9=>'frd.DocumentReceived', 11=>'frd.CheckInProcessingDate'];
   ...
   { 
	...
	{ 
        ...
        if(!is_null($fmdId) && $barcodeGenerated){
			$filesQcData = $this->FilesRecordingData->find()->where(['RecId'=>$fmdId])->first(); 
 			$filesQcData->barcode_generated = 'Y';  
			$this->FilesRecordingData->save($filesQcData); 
        }
    ...
    public function index()
    {
        ...
        //$this->set('isSearch', $this->FilesRecordingData->isSearch());
       ...
       // $fileCsvMasterData =  $this->_getCsvFileListing();,'fileCsvMasterData'
        ...
        $this->set(compact('FilesRecordingData','companyMsts','DocumentTypeData','partnerMapField', 'vendorlist', 'vendor_services'));
	...
	public function _documentReceived($postdata, $DocumentReceived=''){
		...
		if(!empty($value)){
			...
			if(is_array($processRecords))
			{
				...
				$partnerMapData = $this->FilesRecordingData->partnerExportFields($companyId,'cef_fieldid4CHI','checkinsheet');
				...
				$listRecord = $this->FilesRecordingData->searchByFileMainIdDocType($fileMainDataId,$fileDocType, $partnerMapFields);
				...
				if(count($docTypesarr['docType']) > 1){
					...
					if(in_array($docTypesarr['hiddenDocType'], $docTypesarr['docType'])){
						...
						foreach($docTypesarr['docType'] as $newdt)
						{
							...
							{
								// doc type not same
								$exists = $this->FilesRecordingData->exists(['RecId' => $fmd_id, 'transaction_type' => $newdt]);
								if(!$exists){
									//new value NOT exist
									// insert checkin data
									$this->FilesRecordingData->insertNewCheckinData($fmd_id,$newdt,$this->currentUser->user_id,$DocumentReceived);
									// insert public 
									...
									$arr = $this->FilesRecordingData->updateCheckInData($newdt,$DocumentReceived,$fmd_id);
							...
							{
								// same doctype
								$arr = $this->FilesRecordingData->updateCheckInData($newdt,$DocumentReceived,$fmd_id,$docTypesarr['hiddenDocType']);
								// public data 
								$this->PublicNotes->insertNewPublicNotes($fmd_id, $newdt, $this->currentUser->user_id, 'Checkin: Record Updated','Fcd',true);
							}
					...
					foreach($docTypesarr['docType'] as $newdt)
					{
						...
						if($newdt != $docTypesarr['hiddenDocType'])
						{
							...
							//if(!$exists){
								...
								$this->FilesRecordingData->updateCheckInData($newdt,$DocumentReceived,$fmd_id,$docTypesarr['hiddenDocType']);
    ...
    public function delete($id = null)
    {
		...
		$FilesRecordingData = $this->FilesRecordingData->get($id);

          if ($this->FilesRecordingData->delete($FilesRecordingData)) {
			// delete row from main data if deleted records is last records from check in.
			$countMainData = $this->FilesRecordingData->countByFileId($FilesRecordingData->RecId);
			if($countMainData < 1){
				$this->FilesMainData->deleteAll(['Id'=>$FilesRecordingData->RecId]);
			}
			
			// delete related records from other table.
			$this->deleteOtherRecords($FilesRecordingData->RecId,  $FilesRecordingData->transaction_type);
	...
	private function setFilterQuery($requestFormdata, $pdata, $documentReceived='', $selectedIds=null, $documentProcess=''){
		...
		//------DocumentProcess -------------
		$whereConditionDocProcess = ['IFNULL(frd.RecId, 0) =' => '0'];
		
		if($documentProcess == "processed"){
			$whereConditionDocProcess = ['frd.RecId >' => '0'];
		}
		...
		if(!is_null($selectedIds)){
			...
			$whereCondition = 	array_merge($whereCondition, ['frd.RecId IN' => $selectedIds['fmd'], 'fmd.TransactionType IN' => $selectedIds['doc']]);
		}
		...
		$query = $this->FilesRecordingData->filecheckinQuery($whereCondition, $pdata, $cm_partner_cmp);
		
		if(isset($fromdate) && isset($todate)){
 
			//$query = $this->FilesRecordingData->dateBetween($query, $fromdate, $todate, 'fcd.CheckInProcessingDate');
			$query = $this->FilesRecordingData->dateBetween($query, $fromdate, $todate, 'frd.created');
			//debug($query); 
	...
	public function postDataCondition(array $postData, $fields=false){ 
	   ...
	   if(!$fields){
		   ...
		   $this->columns_alise["checkinId"] = "frd.Id";
	   ...
	   //$this->columns_alise["lock_status"] = "frd.lock_status";
	...
	public function ajaxListIndex(){
		...
		// no groupby add
		$recordsTotal = $this->FilesRecordingData->getQueryCountResult($query, 'count', false); 
    ...
    public function add($companyId = null)
    {
        ...
        if ($this->request->is('post')) {
			...
			{ 
				...
				if($countyDetailsCount >= 1)
				{
					...
					$sqlmainInt = $this->FilesRecordingData->sqlDataInsertByForm($postData, $this->currentUser->user_id,$countyDetails['cm_file_enabled'],$nat_file_number);
					...
					if($this->FilesMainData->save($filesMainDataEnter))
					{
						...
						foreach($docTypesarr as $docType){
							...
							//$extensionDT  = $this->FilesRecordingData->getMainDataByMultiDocType($insId, $doctype, 'Id');
 							//Insert in CheckIn with transaction_type		
							$this->FilesRecordingData->insertNewCheckinData($insId, $docType, $this->currentUser->user_id, $this->request->getData('DocumentReceived'));
        ...
        $this->set(compact('FilesRecordingData', 'companyMsts','partnerMapFields','stateList','countyList')); // 'documentList'
        $this->set('_serialize', ['FilesRecordingData']);
    }
	...
	public function setnat_file_number($is_loop = false)
	{
		...
		if(empty($this->setmaxLRSno)){ 
			...
			//$existMaxLRS = $this->FilesRecordingData->FilesMainData->exists(['nat_file_number'=>$this->setmaxLRSno]);
    ...
    public function edit($id = null)
    {
		...
		$FilesRecordingData = $this->FilesMainData->getFileMainData($id); // checkIn id
 
		if(!empty($FilesRecordingData)){
			$fmd_id = $FilesRecordingData['Id']; // file main data id
			$companyId = $FilesRecordingData['company_id']; // need to discuss
    ...
    public function recordingData($id = null)
    {
		// set page title
        $pageTitle = 'Recording Data';
        $this->set(compact('pageTitle')); 
		...
		$sqlmainInt = [];
		$FilesMainData = $this->FilesMainData->getFileRecordingMainData($id);
		...
		if(!empty($FilesMainData)) {
			...
			$recordingDataFields = $this->FilesRecordingData->getMainDataAll($fmd_id);
			$partnerMapFields = $this->CompanyFieldsMap->partnerMapFields($companyId);
			//echo "<pre>";print_r($recordingDataFields);exit;
	        if ($this->request->is(['post', 'put'])) {
				...
				if(isset($saveOpenBtn) || isset($saveBtn))
				{ 
					$examReceipt = $this->FilesRecordingData->newEmptyEntity();
					$hasExamReceipt = $this->FilesRecordingData->getRecordData($fmd_id);
			        ...
			        if ($this->request->is(['post', 'put'])) {
        				...
        				$examReceiptData['RecId'] = $fmd_id;
        				$examReceiptData['user_id'] = $this->currentUser->user_id;
			            $examReceipt = $this->FilesRecordingData->patchEntity($examReceipt, $examReceiptData);
			            if ($this->FilesRecordingData->save($examReceipt)) {
        ...
        $this->set(compact('FilesRecordingData','FilesMainData','recordingDataFields','open_mortgage_attributes','open_judgments_attributes','vesting_attributes','companyMsts','partnerMapFields','stateList','countyList')); // 'documentList'
        $this->set('_serialize', ['FilesRecordingData']);
	...
	private function saveSingleDocument($postData){
		...
		$countFileCheckIn = $this->FilesRecordingData->checkCountByFileIdDoctype($fmd_id, $docType);
		...
		if($countFileCheckIn < 1)
		{
			...
			// assume typeHiddenArr is single value
			$arr = $this->FilesRecordingData->updateCheckInData($docType,$postData['DocumentReceived'],$fmd_id,$typeHiddenArr);
	...
	private function saveMultiSameDocument($postData){
		...
		foreach($docTypesarr as $newdt)
		{
			...
			if($newdt != $typeHiddenArr)
			{
				...
				$countFileCheckIn = $this->FilesRecordingData->checkCountByFileIdDoctype($fmd_id, $newdt);
		
				if($countFileCheckIn < 1){
					// check for extension NEW
					//$extensionDT  = $this->FilesRecordingData->getMainDataByMultiDocType($fmd_id, $newdt, 'Id'); 
					 // insert checkin data
					 $this->FilesRecordingData->insertNewCheckinData($fmd_id,$newdt,$this->currentUser->user_id,$postData['DocumentReceived']);
			...
			}else
			{
				...
				$arr = $this->FilesRecordingData->updateCheckInData($newdt,$postData['DocumentReceived'],$fmd_id,$typeHiddenArr);
				// internal
				$this->PublicNotes->insertNewPublicNotes($fmd_id, $newdt, $this->currentUser->user_id, $regarding,'Fcd'); //$postData['Public_Internal']
 			}
	...
	private function saveFirstDocument($postData){
		...
		foreach($docTypesarr as $newdt)
		{
			$newdt = $this->setDocType($newdt);
			//$countFileCheckIn = $this->FilesRecordingData->checkCountByFileIdDoctype($fmd_id, $newdt);
			$exist = $this->FilesRecordingData->exists(['RecId' => $fmd_id, 'transaction_type' => $newdt]);
			$alexist = 1;
			if(!$exist)
			{
				$alexist = 0;
	...
	public function searchRecords()
    {
       ...
       // $fileCsvMasterData =  $this->_getCsvFileListing();,'fileCsvMasterData'
        ...
        $this->set(compact('FilesRecordingData','companyMsts','DocumentTypeData','partnerMapField'));
        $this->set('_serialize', ['FilesRecordingData']);
	...
	public function ajaxLockRecord(){
		...
		$checkinData = $this->FilesRecordingData->updateLockRecord($formdata['chechinId'], $formdata['lock_status']);
    ...
    public function view($id = null)
    {
		...
		$FilesRecordingData = $this->FilesRecordingData->getCheckInData($recordMainId, $doctype); 
        ...
        $this->set(compact('FilesRecordingData', 'filesMainData', 'documentData', 'documentDataList','partnerMapField'));
    ...
    */
    public function masterView($id = null)
    {
		...
		if(!empty($FilesRecordingData)){
			$fmd_id = $FilesRecordingData['Id']; // file main data id
			$companyId = $FilesRecordingData['CompanyId']; // need to discuss

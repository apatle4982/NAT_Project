...
class FilesVendorAssignmentController extends AppController
{
   ...
   { 
    ...
    public function aolindex(){
		...
		if(isset($docstatus) && (!($this->user_Gateway))){
             ...
             foreach($pd['checkAll'] as $key=>$value){
                $aol['RecId'] = $value;
                if($pd['pre_aol']){ $aol['pre_aol_status'] = "Y"; $pdfController->generatePdf($aol['RecId'], "pre");}else if($pd['final_aol']){ $aol['final_aol_status'] = "Y";$pdfController->generatePdf($aol['RecId'],"final");
    ...
    /*public function essindex(){
		...
		if(isset($docstatus) && (!($this->user_Gateway))){
             ...
             foreach($pd['checkAll'] as $key=>$value){
                ...
                if ($this->request->is('post')) {
                  $existingRecord = $this->FilesEscrowAssignment->find()->where(['RecId' => $aol['RecId']])->first();
                  //echo "<pre>";print_r($existingRecord);echo "</pre>";
                  if ($existingRecord) {
                      // If record exists, patch data for editing
                      $filesAolAssignment = $this->FilesEscrowAssignment->patchEntity($existingRecord, $aol);
                  } else {
                      // If record doesn't exist, create a new one
                      $filesAolAssignment = $this->FilesEscrowAssignment->newEmptyEntity();
                      $filesAolAssignment = $this->FilesEscrowAssignment->patchEntity($filesAolAssignment, $aol);
                  }
                  // Save the record
                  if ($this->FilesEscrowAssignment->save($filesAolAssignment)) {
                      //$this->Flash->success(__('The file AOL assignment has been saved.'));
                  }
              }
              $this->set(compact('filesAolAssignment'));
    ...
    private function setFilterQueryAol($requestFormdata, $pdata, $documentReceived='', $selectedIds=null){
		$whereCondition = [];
        //echo "--".$documentReceived;exit;
		if($documentReceived == "pending"){
			$whereCondition = ['faol.pre_aol_status' => 'N','faol.final_aol_status' => 'N'];
		}
    ...
    private function getCustomParshingDataAol(array $data){
		...
		foreach ($data as $key => $value) {
			...
			$value['DocStatus'] = (($aol_data['pre_aol_status']=='Y')?'Y':'N');
			$value['DocStatus1'] = (($aol_data['final_aol_status']=='Y')?'Y':'N');
			$value['PartnerFileNumber'] = $value['PartnerFileNumber'] . ((!empty($value['ClientCompName'])) ? ' ( '.$value['ClientCompName'].' )': '' );
		 	$value['DateAdded'] = (($value['DateAdded']!='')? '<span '.$dtColor.'>'.date('Y-m-d H:i:s', strtotime((string)$value['DateAdded'])).'</span>' : '');
    ...
    public function add(){
        if ($this->request->is('post')) {
            ...
            // Fetch data from nat_services table
    ...
    public function attadd(){
        ...
        if ($this->request->is('post')) {
            ...
            //echo "<pre>";print_r($natService);echo "</pre>";exit;
            // Fetch vendor details
            $this->loadModel('Vendors');
            $vendor = $this->Vendors->get_vendor($data['vendorid']);
            $data['vendor'] = $vendor->name;
            ...
            if (!empty($data['RecId']) && strpos($data['RecId'], ',') !== false) {
                ...
                foreach ($recIds as $recId) {
                    ...
                    if ($existingRecord) {
                        ...
                        $filesVendorAssignment = $this->FilesAttorneyAssignment->patchEntity($filesAolAssignment, $data);
                    }
            ...
            if (!empty($data['RecId']) && strpos($data['RecId'], ',') !== false) {
                ...
                foreach ($recIds as $recId) {
                    ...
                    if ($existingRecord) {
                        ...
                        $filesVendorAssignment = $this->FilesEscrowAssignment->patchEntity($filesAolAssignment, $data);
                    }
    ...
    public function addRecords($companyId = null)
    {
        ...
        $this->set(compact('FilesVendorAssignment', 'companyMsts','partnerMapFields','StateList','CountyList', 'vendorlist','vendor_services')); // 'documentList'
        $this->set('_serialize', ['FilesVendorAssignment']);
    }
    ...
    private function sendEmail($recipientEmail, $csvDownloadLink, $csvFilePath){
        $mailer = new Mailer();
    ...
    private function saveCsvFile($dataArray){
        ...
        $csvColumns = [
            'RecId', 'vendor', 'vendorid', 'search_criteria_name',
            'search_criteria', 'delivery_type', 'turn_around_datetime'
        ];

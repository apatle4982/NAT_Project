...
class MasterDataController extends AppController
{
	...
	public function initialize(): void
	{
		...
		// Get logged-in identity wrapper
        $identity = $this->Authentication->getIdentity();
        // Get actual User entity (with isSuperAdmin(), etc.)
	...
	private function queryWithProcessDate($requestFormdata, $pdata, $whereCondition, $skipJoin, $processingstatus){
		...
		$dateFldTable = ['fmd'=>"fmd.DateAdded", 'fva'=>"fva.date_updated", 'fqcd'=>"fqcd.QCProcessingDate", 'fsad'=>"fsad.ShippingProcessingDate", 'fad'=>"fad.AccountingProcessingDate", 'frd'=>"frd.RecordingProcessingDate", 'frtp'=>"frtp.RTPProcessingDate"];
    ...
    */
    public function masterView($id = null)
    {
		...
		$AttorneyReviews = $this->AttorneyReviews->find()->where(['RecId' => $id])->first();
		...
		$fileVendorAssignment = $this->FilesVendorAssignment->find()->where(['RecId' => $id])->first();
		...
		$this->set(compact('pageTitle', 'filesCheckinData','AttorneyReviews','fmd_data','AttData','AolData','filesExamReceiptNew','fileVendorAssignment'));

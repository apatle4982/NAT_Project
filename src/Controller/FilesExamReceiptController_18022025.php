...
class FilesExamReceiptController extends AppController
{
	...
	private $orig_fields = ['file_start_date','PartnerID','nat_file_number','partner_file_number','grantors_g1','first_name_1_g1','first_name_2_g1','grantees_g2','street_number','street_name','city','county','state','zip','transaction_type','CheckInDateTime','CountyRecordingFee','Taxes','AdditionalFees','Total','CarrierName','CarrierTrackingNo','RecordingDate','RecordingTime','InstrumentNumber','Book','Page','PublicNotes','loan_amount','File','vested_deed_type'];
	...
	private  $columnsorder = [0=>'fmd.Id', 1=>'fmd.NATFileNumber', 2=>'fmd.PartnerFileNumber', 3=>'fer.extension', 4=>'fmd.TransactionType', 5=>'fmd.Grantors', 6=>'fmd.StreetName', 7=>'fmd.county', 8=>'fmd.State', 9=>'fer.DocumentReceived', 11=>'fer.CheckInProcessingDate'];
   ...
   { 
	...
	{ 
        $fileno = $this->request->getData('fileno'); //nat_file_number
        $doctype = $this->request->getData('doctype');
       
        $query =  $this->FilesMainData->find()
                ->select('Id')
                ->where(['nat_file_number' => $fileno]);
			...
			if(!empty($listTownError)){ 
				...
				foreach($listTownError as $listTown){
					...
					$towstxt .='<tr>
									...
									<td>'.(($listTown['nat_file_number']) ? substr($listTown['nat_file_number'],0,65) : '').'</td>
	...
	private function setFilterQuery($requestFormdata, $pdata, $documentReceived='', $selectedIds=null, $documentProcess=''){
		//=====================filter conditions===============================================
		//------DocumentReceived -------------
		$whereCondition = ['fer.DocumentReceived in' => ['Y', 'Y']];
		
		if($documentReceived == "dr"){
			$whereCondition = ['fer.DocumentReceived =' => 'Y'];
		}
	...
	public function ajaxListIndex(){
		...
		$pdata = $this->postDataCondition($this->request->getData()); //print_r($pdata);exit;
    ...
    private function getCustomParshingData(array $data){
		...
		foreach ($data as $key => $value) {
			...
			if($this->user_Gateway){
				...
				$value['transaction_type'] = '<input type="hidden" name="documentTypeHidden" value="'.$value["transaction_type"].'"><input style="width:40px;" type="text" name="docTypeInput" class="docinput" value="'.$value["transaction_type"].'" /> ( '.$value["DocumentTitle"].' )';  
	...
	public function downloadSamplefile() {
		...
		$table = 'CenterBranch,loan_amount,file_start_date,ClientId,nat_file_number,partner_file_number,grantors_g1,first_name_1_g1,first_name_2_g1,grantees_g2,first_name_1_g2,street_name,street_name,city,state,apn_parcel_number';
    ...
    public function examReceipt($id = null)
    {
		...
		if(!empty($FilesMainData)) {
			...
			$partnerMapFields = $this->CompanyFieldsMap->partnerMapFields($companyId);
			//echo "<pre>";print_r($examReceiptFields);exit;
        ...
        $this->set(compact('FilesExamReceipt','FilesMainData','examReceiptFields','companyMsts','partnerMapFields','stateList','countyList')); // 'documentList'
        $this->set('_serialize', ['FilesExamReceipt']);
	...
	private function partnerMapFields($companyId){
		$partnerMapFields = ['nat_file_number'=>'','partner_file_number'=>'','grantors_g1'=>'','street_name'=>'','county'=>'','state'=>''];
		$companyExportField =[];
		//===== generete csv file data & name to export data====================//	
		$partnerMapData = $this->FilesExamReceipt->partnerExportFields($companyId,'cef_fieldid4CHI','checkinsheet');
		$partnerMapFields = $partnerMapData['partnerMapFields'];
		...
		if($this->currentUser->user_Gateway && isset($partnerMapFields['nat_file_number'])){
			unset($partnerMapFields['nat_file_number']);
		}

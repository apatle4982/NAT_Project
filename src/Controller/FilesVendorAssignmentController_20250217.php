...
class FilesVendorAssignmentController extends AppController
{
   ...
   { 
    ...
    public function generateCsv($rec_ids) {
        ...
        //PDF generation for multiple ids start
        /*$tempFolder = WWW_ROOT . 'files/export/aol_assignment/';
        if (!is_dir($tempFolder)) {  mkdir($tempFolder, 0777, true); }

        $zip = new ZipArchive();
        $zipFile = TMP . 'all.zip';

        if ($zip->open($zipFile, ZipArchive::CREATE) !== TRUE) { $this->Flash->error(__('Cannot open ZIP file.')); return; }
        echo "<pre>";print_r($rec_ids);echo "</pre>";exit;
        foreach ($rec_ids as $id) {
            $data = $this->FilesExamReceipt->find()->where(['RecId' => $id])->first();
            if (!$data) { continue; }
            $pdfFilePath = $tempFolder . "AssignmentDetails-$id.pdf";
            $pdf->AddPage();
            $pdf->Output('F', $pdfFilePath);

            $zip->addFile($pdfFilePath, "AssignmentDetails-$id.pdf");
        }
        $zip->close();
        array_map('unlink', glob($tempFolder . "*.pdf"));
        rmdir($tempFolder);

        $this->Flash->success(__('The PDFs have been generated successfully and are zipped. <a href="/files/tmp/all.zip" target="_blank">Click here to download the ZIP file.</a>'));
        ...
        //PDF generation for multiple ids end
    ...
    public function add(){
        if ($this->request->is('post')) {
            ...
            if ($natService) {
                ...
                $this->Flash->error(__('No record found in nat_services with id ' . $data['search_criteria'] . '.'));
                return;
            }

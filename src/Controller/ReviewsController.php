...
class ReviewsController extends AppController
{
    ...
    public function initialize(): void
    {
        ...
        $this->Authentication->allowUnauthenticated(['index', 'add','docusign','callback']);
    }
    ...
    public function index($id = null){
    	...
    	$companyMsts = $this->CompanyMst->find()->where(['cm_id' => $companyId])->first(); 
        ...
        $this->set(compact('FilesMainData','examReceiptFields', 'attorney_dtl', 'examReceiptFields1','attorneyReviewDtl','companyMsts'));
    ...
    public function callback($aolId=null)
    {
        ...
        try {
            ...
            $partner = $this->loadModel('FilesMainData')->find()->where(['Id' => $aolId])->first();
            $company = $this->CompanyMst->find()->select(['cm_email'])->where(['cm_id' => $partner->company_id])->first();
            $filePath = WWW_ROOT . 'files' . DS . 'export' . DS . 'aol_assignment' . DS . 'pdf' . DS . 'aol_signed' . DS . "AssignmentDetails-".$aolId.".pdf";
            
            if (file_exists($filePath)) {
                //echo "File exists: " . $filePath;
                $this->sendEmailToClient($company->cm_email, $filePath);
            }
            $FmdData = $this->FilesMainData->find()->where(['Id' => $aolId])->first();
            $currentUserId = $this->currentUser->user_id;
            $existingRecord = $this->FilesAolAssignment->find()->where(['RecId' => $aolId])->first();

            if ($existingRecord) {
                // If record exists, patch data for editing
                $filesAolAssignment = $this->FilesAolAssignment->patchEntity($existingRecord, $aol);
                $this->PublicNotes->insertNewPublicNotes($aolId, $FmdData['TransactionType'], $currentUserId, $regarding, 'Aol',true, "Aol Processing");
            } else {

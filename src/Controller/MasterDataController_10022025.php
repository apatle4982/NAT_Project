...
class MasterDataController extends AppController
{
	...
	public $fileSearchType = ['fmd'=>'File / Record Added', 'fcd'=>'Document CheckIn (Received)', 'fad'=>'Accounting', 'fsad'=>'Submission To County', 'fqcd'=>'Open Rejection Date', 'frd'=>'Recorded', 'frtp'=>'Return to Partner Submission'];
	...
	public function initialize(): void
	{
	   ...
	   $this->CRskipJoin = ['files_checkin_data'.ARCHIVE_PREFIX,'files_recording_data'.ARCHIVE_PREFIX,'files_qc_data'.ARCHIVE_PREFIX, 'files_returned2partner'.ARCHIVE_PREFIX];
	   $this->MSskipJoin = ['files_checkin_data'.ARCHIVE_PREFIX, 'files_recording_data'.ARCHIVE_PREFIX];
	}
	...
	private function setExtraFields($type="index"){
	   ...
	   if($type=="master"){
			...
			$this->columns_alise["TransactionType"] = "fcd.TransactionType"; // custome table alise change
			
			unset($this->columnsorder[8]); //$this->columnsorder[5],
			
			$this->columnsorder[3]= "fcd.TransactionType"; // custome table alise change
	...
	private function queryWithProcessDate($requestFormdata, $pdata, $whereCondition, $skipJoin, $processingstatus){
		...
		if(!empty($requestFormdata['StartDate']) || !empty($requestFormdata['EndDate'])){
			...
			$whereCondition = @array_merge($whereCondition,[$this->FilesMainData->dateBetweenWhere($requestFormdata['StartDate'],$requestFormdata['EndDate'], $dateFldTable[$fcd])]);
			// add column to search in query as per selected section
			$this->columns_alise = array_merge($this->columns_alise, [$dateFldTable[$fcd]]);
		}

...
class FilesCheckinDataController extends AppController
{
    ...
    private $columns_alise = [  "Checkbox" => "",
								...
								"Extension" => "fcd.extension",
                                "TransactionType" => "fcd.TransactionType",
                                "Grantors" => "fmd.Grantors",
                                "StreetName" => "fmd.StreetName",
                                "County" => "fmd.County",
                                "State" => "fmd.State",
                                "DocStatus" => "fcd.DocumentReceived",
                                
                                "DateAdded" => "concat(fcd.CheckInProcessingDate, ' ', fcd.CheckInProcessingTime)",
								"Actions" => ""
								
                            ];

	private  $columnsorder = [0=>'fmd.Id', 1=>'fmd.NATFileNumber', 2=>'fmd.PartnerFileNumber', 3=>'fcd.extension', 4=>'fcd.TransactionType', 5=>'fmd.Grantors', 6=>'fmd.StreetName', 7=>'fmd.County', 8=>'fmd.State', 9=>'fcd.DocumentReceived', 11=>'fcd.CheckInProcessingDate'];
   ...
   { 
    ...
    private function getCustomParshingData(array $data){
		...
		foreach ($data as $key => $value) {
			...
			$value['PartnerFileNumber'] = $value['PartnerFileNumber'] . ((!empty($value['ClientCompName'])) ? ' ( '.$value['ClientCompName'].' )': '' );
		 	$value['DateAdded'] = (($value['DateAdded']!='')? '<span '.$dtColor.'>'.date('Y-m-d H:i:s', strtotime($value['DateAdded'])).'</span>' : '');
	...
	public function downloadSamplefile() {
		...
		$table = 'CenterBranch,LoanAmount,FileStartDate,ClientId,NATFileNumber,PartnerFileNumber,Grantors,GrantorFirstName1,first_name_2_g1,Grantees,GranteeFirstName1,GranteeLastName2,StreetName,StreetName,City,State,apn_parcel_number';

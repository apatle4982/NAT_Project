...
class FilesMainDataTable extends Table
{
	...
	public function getFileMainTownShipData($FmdIds=[]){
		...
		if(!empty($FmdIds)){
			$this->setAlias('fmd');
			$query = $this->find()
						->where(['fmd.Id IN '=>$FmdIds, 'fcd.DocumentReceived'=>''])
						->join([
						...
						->select(['fmd.Id', 'fmd.NATFileNumber', 'comp_mst.cm_comp_name', 'dtm.Title', 'fmd.company_id', 'fmd.PartnerFileNumber', 'fmd.Grantors', 'fmd.StreetName','fmd.State', 'fmd.County', 'fmd.ECapable', 'fcd.TransactionType', 'fcd.lock_status'])
	...
	public function recordingKNIcheckDataQuery(array $fmdIdDoctype, $count=false){
		...
		$query = $this->find()
					...
					->join([
						...
						'alias' => 'fcd',
						'type' => 'LEFT OUTER',
						'conditions' => ['fcd.RecId = frd.RecId','fcd.TransactionType = frd.TransactionType']
					]);
    ...
    public function shipCountyQuery($whereCondition, array $pdata){
		...
		$query = $this->find('search', $pdata)
                ...
                ->join([
                    ...
                    'alias' => 'fsad',
                    'type' => 'LEFT OUTER',
                    'conditions' => ['fcd.RecId = fsad.RecId','fcd.TransactionType = fsad.TransactionType']
                ])
				->join([
					'table' => 'document_type_mst',
					'alias' => 'dtm',
            ...
            if(!empty($pdata['search']['AccountingStartDate']) || !empty($pdata['search']['AccountingEndDate'])) {
                $query = $query->join([
                    ...
                    'alias' => 'fad',
                    'type' => 'LEFT OUTER',
                    'conditions' => ['fad.RecId = fcd.RecId']
                ]);
            }   
    ...
    public function getMainDataAll($recordMainId) {
        ...
        $search =[  
                    ...
                    "recording_data.RecordingProcessingDate",
                    "recording_data.RecordingDate",
                    "recording_data.RecordingTime",
                    "recording_data.InstrumentNumber",
                    "recording_data.Book",
                    "recording_data.Page",
        ...
        $query = $this->find()->where(['fmd.Id'=>$recordMainId])
                        ...
                        ->join([
                                ...
                                'alias' => 'recording_data',
                                'type' => 'LEFT',
                                'conditions' => ['recording_data.RecId = fmd.ID']
                            ])
                        ->join([
	...
	public function masterSearchQuery($whereCondition, array $pdata){
		...
		$query = $this->find('search', $pdata)
						...
						->join([
							...
							'alias' => 'frd',
							'type' => 'LEFT OUTER',
							'conditions' => ['frd.RecId = fcd.Id','frd.TransactionType = fcd.TransactionType']
						]);
    ...
    public function filesRecordingQuery($whereCondition, array $pdata, $callpage=NULL){
        ...
        $query = $query->group(['IF(frd.RecId IS NULL,fcd.TransactionType,frd.TransactionType)', 'IF(frd.RecId IS NULL,fcd.RecId,frd.RecId)']);
...
// same as comlpetemaster query
	...
	public function completeMasterQuery($whereCondition, array $pdata){
		...
		$query = $this->find('search', $pdata)
					...
					->join([
						...
						'alias' => 'frtp',
						'type' => 'LEFT',
						'conditions' => ['frtp.RecId = fcd.RecId', 'frtp.TransactionType = fcd.TransactionType']
					])
					->join([
						'table' => 'document_type_mst',
						'alias' => 'dtm',
    ...
    public function queryAfPendingFee($fromDate=''){
		...
		$query = $query->select(
						...
						// diff 180 days
						->where(["DATEDIFF('$fromDate',fcd.CheckInProcessingDate) < " => 180])  
						->join([
                            'table' => 'files_checkin_data'.ARCHIVE_PREFIX,
                            'alias' => 'fcd',
                            'type' => 'INNER',
						...
						->join([
								...
								'alias' => 'fad',
								'type' => 'LEFT OUTER',
								'conditions' => ['fcd.RecId = fad.RecId','fcd.TransactionType = fad.TransactionType']
							])
						->join([
								'table' => 'document_type_mst',
								'alias' => 'dtm',
    ...
    public function pendingFileAnalysisQuery(array $whereCondition, array $selectFld, $prifix='fsad'){
        ...
        if($prifix == 'fsad'){
            $query = $query->join([
                    ...
                    'alias' => 'fsad',
                    'type' => 'LEFT OUTER', 
                    'conditions' => ['fsad.RecId = fcd.RecId','fsad.TransactionType = fcd.TransactionType']
                ]);
        }
        ...
        if($prifix == 'frd'){
            $query = $query->join([
                    ...
                    'alias' => 'frd',
                    'type' => 'LEFT OUTER',
                    'conditions' => ['frd.RecId = fcd.RecId','frd.TransactionType = fcd.TransactionType']
                ]); 
        }
    ...
    public function fetchCompanyFromFile($fileName =''){
        ...
        $query = $this->find()->select(['fmd.company_id', 'frd.Id', 'fcd.RecId','fcd.TransactionType']);
                $query = $query->join([
                    'table' => 'files_checkin_data'.ARCHIVE_PREFIX,
                    'alias' => 'fcd',
                    'type' => 'LEFT OUTER',

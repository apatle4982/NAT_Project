...
class FilesVendorAssignmentTable extends Table
{
	...
	public function searchManagerConfig(){
		return $this->searchManager()
			...
			->Value('company_id', ['prifix' => 'fmd'])
			->Like('NATFileNumber', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Like('PartnerFileNumber', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Like('StreetName', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Like('County', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Like('State', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Like('Grantors', ['prifix' => 'fmd', 'before' => true, 'after' => true])
			->Value('cm_partner_cmp', ['prifix' => 'cpm']);
	}
	...
	public function insertNewCheckinData($fmdLastId,$documentId,$currentUserId,$docRecived=null, $LRS_extension=0, $vendorid=0, $criteria=array()){
		// check unique records for fmd_id & doc_id
		$exist = $this->exists(['RecId' => $fmdLastId, 'TransactionType' => $documentId]);
        if(count($criteria)>0){ $criteria=implode(',', $criteria); }
	...
	public function validationApi(Validator $validator): Validator
	{
		return $validator
			...
			->notEmptyString('company_id', 'Missing required field: company_id')

			->requirePresence('PartnerFileNumber', 'create')
			->notEmptyString('PartnerFileNumber', 'Missing required field: PartnerFileNumber')

			->requirePresence('FileStartDate', 'create')
			->notEmptyDate('FileStartDate', 'Missing required field: FileStartDate')
			//->date('FileStartDate', ['ymd'], 'FileStartDate date must be a valid date in Y-m-d format')

			->requirePresence('TransactionType', 'create')
			->notEmptyString('TransactionType', 'Missing required field: TransactionType')

			->requirePresence('PurchasePriceConsideration', 'create')
			->notEmptyString('PurchasePriceConsideration', 'Missing required field: PurchasePriceConsideration')

			->requirePresence('City', 'create')
			->notEmptyString('City', 'Missing required field: City')

			->requirePresence('State', 'create')
			->notEmptyString('State', 'Missing required field: State')

			->requirePresence('County', 'create')
			->notEmptyString('County', 'Missing required field: County')

			->requirePresence('Zip', 'create')
			->notEmptyString('Zip', 'Missing required field: Zip');
	}
	...
	public function getCheckInData($fmdId, $doctype = ''){
		...
		$query = $this->find()
		->where(['RecId'=>$fmdId])
		->select(['Id','DocumentReceived','date_updated']);

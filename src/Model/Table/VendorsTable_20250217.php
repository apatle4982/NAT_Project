...
class VendorsTable extends Table
{
    ...
    public function get_vendor_services($id) {
        //SELECT ns.id AS id, ns.sub_service FROM vendors v LEFT JOIN nat_services ns ON (ns.id = v.title_exam_services OR ns.id = v.aol_generation OR ns.id = v.full_escrow_closing) where v.id=12;
        $query = $this->find()
        ->select(['id' => 'ns.id', 'sub_service' => 'ns.sub_service', 'time'=>'ns.turn_around_time'])
        ->leftJoin(
            ['ns' => 'nat_services'], // Alias for nat_services
            [
                'OR' => [
                    'ns.id = Vendors.title_exam_services',
                    'ns.id = Vendors.aol_generation',
                    'ns.id = Vendors.full_escrow_closing'
                ]
            ]
        )
        ->where(['Vendors.id' => $id]);

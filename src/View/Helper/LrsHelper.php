...
use Cake\View\Helper;
	...
	class LrsHelper extends Helper
	{
		...
		public function searchCancelBtnAol($class=''){	//$isSearch=false
			$text = '<div class="btn-group pull-right '.$class.'" role="group">';
				$text .= $this->Form->button(__('Search'), ['class'=>'btn btn-primary block m-b','id'=>'searchBtnId']); 
			$text .= '</div>';
			$text .= '<div class="btn-group pull-right mt-2 '.$class.'" role="group">'; 
				$text .= $this->Html->link(__('Clear'), ['action'=>'aolindex'],['class'=>'btn btn-danger flt-rght']); 
			$text .= '</div>';
			return $text;
		}
		
		public function searchCancelBtnAtt($class=''){	//$isSearch=false
			$text = '<div class="btn-group pull-right '.$class.'" role="group">';
				$text .= $this->Form->button(__('Search'), ['class'=>'btn btn-primary block m-b','id'=>'searchBtnId']); 
			$text .= '</div>';
			$text .= '<div class="btn-group pull-right mt-2 '.$class.'" role="group">'; 
				$text .= $this->Html->link(__('Clear'), ['action'=>'attindex'],['class'=>'btn btn-danger flt-rght']); 
			$text .= '</div>';
			return $text;
		}
		
		public function searchCancelBtnEss($class=''){	//$isSearch=false
			$text = '<div class="btn-group pull-right '.$class.'" role="group">';
				$text .= $this->Form->button(__('Search'), ['class'=>'btn btn-primary block m-b','id'=>'searchBtnId']); 
			$text .= '</div>';
			$text .= '<div class="btn-group pull-right mt-2 '.$class.'" role="group">'; 
				$text .= $this->Html->link(__('Clear'), ['action'=>'essindex'],['class'=>'btn btn-danger flt-rght']); 
			$text .= '</div>';
			return $text;
		}	   
        ...
        public function showBarCodeModelPop($vendorlist, $vendor_services){
            ...
            $vendor_select = $this->Form->select('vendorid',$vendorlist,['empty' => 'Select Vendor','class' =>'js-example-basic-single form-control', 'required'=>'required', 'label' => false, 'id' => 'vendorid']);

		    $vendor_services_html='<div class="row gy-4">
                        <div id="serv"><label>Services: </label> Select Vendor</div>
		                </div>';
            ...
            /*$vendor_services_html .='<br><div class="form-group"><b>CC National Vendor: </b>
                <input type="checkbox" class="form-check-input" id="cc_nat_vendor" name="cc_nat_vendor" value="email" checked>
                <input class="js-example-basic-single form-control" id="cc_nat_vendor_email" name="cc_nat_vendor_email" value="vsurjuse@tiuconsulting.com"><br>';*/
            ...
            '.$vendor_services_html.'
						...
						<div class="modal-footer">
							...
							'.$this->Form->button(__('Submit'), ['class' => 'btn btn-success']).'<button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
        ...
        public function showBarCodeModelPopAtt($vendorlist, $vendor_services){
            ...
            $vendor_select = $this->Form->select('vendorid',$vendorlist,['empty' => 'Select Attorney','class' =>'js-example-basic-single form-control', 'required'=>'required', 'label' => false, 'id' => 'vendorid']);

		    $vendor_services_html='<div class="row gy-4">
                        <div id="serv"><label>Services: </label> Select Attorney</div>
		                </div>';
            /*foreach ($vendor_services as $result) {
                if($result->time == "NA"){ $time="NA"; }else if($result->time == 0){ $time="Same Day"; }else{ $time = $result->time." Hours"; }
                $vendor_services_html .= '<div class="form-group">
        ...
        public function showBarCodeModelPopEss($vendorlist, $vendor_services){
            ...
            $vendor_select = $this->Form->select('vendorid',$vendorlist,['empty' => 'Select Escrow Service','class' =>'js-example-basic-single form-control', 'required'=>'required', 'label' => false, 'id' => 'vendorid']);

		    $vendor_services_html='<div class="row gy-4">
                        <div id="serv"><label>Services: </label> Select Escrow Service</div>
		                </div>';
            /*foreach ($vendor_services as $result) {
                if($result->time == "NA"){ $time="NA"; }else if($result->time == 0){ $time="Same Day"; }else{ $time = $result->time." Hours"; }
                $vendor_services_html .= '<div class="form-group">
        ...
        public function showPasswordModelPopAol(){
			$text = '<div class="modal fade aol_del" id="myModalPassword" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
					<div class="modal-dialog" role="document">
						...
						<div class="modal-content">
							...
							'. $this->Form->create(null, ['url' => ['controller' => 'FilesVendorAssignment', 'action' => 'aoldelete']]).'
							<div class="modal-body" id="lock-div">
								<div class="col-xxl-12 col-md-12">
									'.$this->Form->control('checkinId',['type'=>'hidden','id'=>'checkinId','value'=>'']).'
